...
//        Color[] colors = new Color[] {Color.red, new Color(172,101,0), new Color(43,164,0), Color.green};
//        Color[] colors = new Color[] {Color.black, Color.BLUE, Color.cyan, Color.green, Color.yellow, Color.red, Color.white};
    ...
    public void makeHeatMap(JEXData[][] xos)
    {
        ...
        for (int i=0, leni=xos[0].length; i<leni; i++){
			for (int j=0, lenj=xos.length; j<lenj; j++){
				...
				// If the value is null set the value in the array to the mean value
				if (value == null || value.isNaN() || value.isInfinite()){
					data[i][j] = Double.NaN;
					continue;
				}
        ...
        if (percent)
        {
        	...
        	for (int i=0, leni=data[0].length; i<leni; i++){
        		for (int j=0, lenj=data.length; j<lenj; j++){
        			...
        			if (data[i][j].equals(Double.NaN)) data[i][j] = meanValue;
        ...
        if (meanRange.equals(Double.NaN)) meanRange = meanValue;
        else meanRange = (meanRange > maxRange) ? maxRange : (meanRange < minRange) ? minRange : meanRange;
...
//        panel = new HeatMap(data, useGraphicsYAxis, redToGreen);
    ...
    public ImagePlus getHeatMapImage(int width, int height){
		...
		// solution 2
//    	java.awt.image.BufferedImage image = (java.awt.image.BufferedImage)panel.createImage(width , height);
