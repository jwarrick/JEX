...
public class Exporter {
	...
	public JEXData[][] makeArrayFromList(TreeSet<JEXEntry> entryList, TypeName dataTN){
		...
		for (JEXEntry entry: entryList){
			...
			if (data != null) {
				...
				if (row == null) row = new TreeMap<Integer,JEXData>();
				row.put(j, data);
				dataarray.put(i, row);
				
				// Place the entry in the hash map
				TreeMap<Integer,JEXEntry> entryrow = entryarray.get(i);
				if (entryrow == null) entryrow = new TreeMap<Integer,JEXEntry>();
	...
	public void exportAsCSV(){
		...
		if (this.exportMode == EXPORT2D){
			...
			for (int i=0, leni=exportArray[0].length; i<leni; i++){
				for (int j=0, lenj=exportArray.length; j<lenj; j++){
					...
					if (xd == null) result = result + ",";
					else result = result + "," + xd.toString();
				}
				result = result + "\n";
	        }
		}
		...
		if (saveAsObject){
			JEXStatics.logManager.log("Printing csv array to file and saving in databse as jexfile",1,this);
			if (objectName.equals("")) return;
			...
			for (JEXEntry entry: entryList)
			{
				...
				JEXData data2add   = FileWriter.makeFileObject(objectName, objectPath,"Export");
			...
			JEXStatics.logManager.log("Printing csv array to file",1,this);
			String objectPath = JEXWriter.saveText(result, "csv");
			try{
	...
	public void exportAsImage(){
		if (dataTN == null || entryList == null) return;
		
		String type = "";
		for (JEXEntry entry: entryList){
			JEXData  data   = JEXStatics.jexManager.getDataOfTypeNameInEntry(dataTN, entry);
			if (data == null) continue;
			String datatype = data.getDataObjectType();
			if (!type.equals("") && !type.equals(datatype)) {
				JEXStatics.logManager.log("All the types don't match... exiting",1,this);
				JEXStatics.statusBar.setStatusText("Export error");
				return;
			}
			if (datatype.equals(JEXData.IMAGE)) type = JEXData.IMAGE;
			else if (datatype.equals(JEXData.VALUE)) type = JEXData.VALUE;
		}
		if (type.equals("")){
			JEXStatics.logManager.log("Cannot export this type of data",1,this);
		...
		if (type.equals(JEXData.VALUE))
		{
			if (this.exportMode == EXPORT2D)
			{
				...
				{
					JEXStatics.logManager.log("Error occrued in creation of heat map",1,this);
					JEXStatics.statusBar.setStatusText("Export error");
				}
			}
			else 
...
//							imageArray[c][r] = null;
							...
							if (data == null) imageArray[c][r] = null;
							else 
							{
				...
				{
					JEXStatics.logManager.log("Error occrued in creation of the montage",1,this);
					JEXStatics.statusBar.setStatusText("Export error");
				}
			}
		}
		...
		// Save the image
		if (im2Save == null) {
			JEXStatics.logManager.log("Null image created... error",1,this);
			JEXStatics.statusBar.setStatusText("Export error");
			return;
		}
		...
		if (saveAsObject)
		{
			...
			String objectPath = JEXWriter.saveImage(im2Save);
			JEXStatics.logManager.log("Printing image montage to file " + objectPath,1,this);
