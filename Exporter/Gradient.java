...
public class Gradient
{
    ...
    /**
     * Creates a gradient from red to green with a black transition at the index INDEXOFMEAN
     * If INDEXOFMEAN is <= 0, then the red section will not show 
     * If INDEXOFMEAN is >= 0, then the green section will not show
     * @param numSteps
     * @param indexOfMean
     * @return
     */
    public static Color[] createRedToGreenGradientWithSetMean(final int numSteps, int indexOfMean)
    {   	
    	// Determine starting red values
    	// If the index of the black transition is less then 0 then do not show the red
    	int r1 = (indexOfMean <= 0) ? Color.BLACK.getRed() : Color.RED.getRed();
    	int g1 = (indexOfMean <= 0) ? Color.BLACK.getGreen() : Color.RED.getGreen();
    	int b1 = (indexOfMean <= 0) ? Color.BLACK.getBlue() : Color.RED.getBlue();
    	int a1 = (indexOfMean <= 0) ? Color.BLACK.getAlpha() : Color.RED.getAlpha();
    	if (indexOfMean <= 0) indexOfMean = 0;

    	// Determine intermediate black values
    	int r2 = Color.BLACK.getRed();
    	int g2 = Color.BLACK.getGreen();
    	int b2 = Color.BLACK.getBlue();
    	int a2 = Color.BLACK.getAlpha();

    	// Determine ending green values
    	// If the index of the black transition is more than the max, then do not show the green
    	int r3 = (indexOfMean >= numSteps) ? Color.BLACK.getRed() : Color.GREEN.getRed();
    	int g3 = (indexOfMean >= numSteps) ? Color.BLACK.getGreen() : Color.GREEN.getGreen();
    	int b3 = (indexOfMean >= numSteps) ? Color.BLACK.getBlue() : Color.GREEN.getBlue();
    	int a3 = (indexOfMean >= numSteps) ? Color.BLACK.getAlpha() : Color.GREEN.getAlpha();
    	if (indexOfMean >= numSteps) indexOfMean = numSteps;
    	...
    	// Create the red gradient on the first section of the array
    	for (int i = 0; i < indexOfMean; i++)
    	{
    		iNorm = i / (double)indexOfMean; //a normalized [0:1] variable
    		newR = (int) (r1 + iNorm * (r2 - r1));
    		newG = (int) (g1 + iNorm * (g2 - g1));
    		newB = (int) (b1 + iNorm * (b2 - b1));
    		newA = (int) (a1 + iNorm * (a2 - a1));
    	...
    	// Create the green gradient for the last section of the array
    	for (int i = indexOfMean; i < numSteps; i++)
    	{
    		iNorm = (i-indexOfMean) / (double)(numSteps-indexOfMean); //a normalized [0:1] variable
    		newR = (int) (r2 + iNorm * (r3 - r2));
    		newG = (int) (g2 + iNorm * (g3 - g2));
    		newB = (int) (b2 + iNorm * (b3 - b2));
    		newA = (int) (a2 + iNorm * (a3 - a2));
    		gradient[i] = new Color(newR, newG, newB, newA);
    	}
