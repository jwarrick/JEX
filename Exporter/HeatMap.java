...
public class HeatMap extends JPanel
{
   ...
   // private double  meanValue     = Double.NaN;
    ...
    /**
     * Make the heat map with the dataarray and color gradient provided
     * @param data
     * @param useGraphicsYAxis
     * @param colors
     */
    public void makeHeatMap(Double[][] data, boolean useGraphicsYAxis, Color[] colors)
    {
    	 updateGradient(colors);
         updateData(data, useGraphicsYAxis);
	...
	/**
	 * Set the range of the values and set the HeatMap to be drawn from a manual entered range
	 * @param min
	 * @param mean
	 * @param max
	 */
    ...
    private void updateDataColors()
    {
    	...
    	// If the range has been pre-set manualll use it 
    	if (this.useFixedRange)
    	{
    		largest  = this.maxValue;
    		smallest = this.minValue;
    	}
