...
public class ZVI_Reader {
	boolean littleEndian = true;
	@SuppressWarnings("unused")
	private int theC, theZ, numC;
	//+ (mb)    
	//+ new code
	private Set<Integer> C_Set = new HashSet<Integer>(); // to hold C channel index collection
	//- (mb)    
	...
	/** Executes the plugin. */
	public Hashtable<Integer,ImageStack> decodeZVI(String zviFileName) {
		if (zviFileName == null) return null;
		File zviFile     = new File(zviFileName);
		this.fileName    = zviFile.getName();
		String directory = zviFile.getParent() + File.separator;

		JEXStatics.logManager.log("Opening: " + directory + fileName, 1, this);
		FileInfo[] fi = null;
		try 
		{ 
			...
			JEXStatics.logManager.log("ZVI Reader error",1,this);
			return null;
		}
		if (fi == null) {
			JEXStatics.logManager.log("Could not find header information",1,this);
			return null;
		}
		...
		if (imp == null) {
			JEXStatics.logManager.log("Could not extract pixel data.",1,this);
			return null;
		}
...
//		String stackName="";
		...
		Hashtable<Integer,ImageStack> img = new Hashtable<Integer,ImageStack>();
		Iterator<Integer> it = C_Set.iterator();
		while (it.hasNext()) {
			Object theC = it.next();
			img.put((Integer)theC, new ImageStack (sWidth,sHeight));
		}
...
//		//- (mb)
	...
	throws IOException
	{
		...
		//+ (mb)    
		//+ new code
		Set<Integer> Z_Set = new HashSet<Integer>(); // to hold Z plan index collection.
		Set<Integer> T_Set = new HashSet<Integer>(); // to hold T time index collection.
		//- (mb)    
		long pos = 0;
		Vector<ZVIBlock> blockList = new Vector<ZVIBlock>();
		int numZ = 0, numT = 0;
		numC=0;
		while (true) {
		...
		if (numZ * numC * numT != blockList.size()) {
			JEXStatics.logManager.log("Warning: image counts do not match.",1,this);
		}
		...
		for (int i=0; i<fi.length; i++) {
			...
			if (ft < 0) {
				JEXStatics.logManager.log("Warning: unknown file type for image plane #" + (i + 1),1,this);
	...
	private class ZVIBlock {
				...
				long imagePos)
		{
			...
			if (bytesPerPixel % numChannels != 0) {
				JEXStatics.logManager.log("Warning: incompatible bytesPerPixel (" +
						bytesPerPixel + ") and numChannels (" + numChannels +
				"). Assuming grayscale data.",1,this);
				numChannels = 1;
			}
