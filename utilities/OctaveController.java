...
public class OctaveController {
	...
	public void makeEngine(String workingDirectory)
	{
		...
		try {
			...
			JEXStatics.logManager.log("Octave engine created !", 0, this);
		}
		catch (Exception e){
//			JEXStatics.logManager.log("Error ! Octave engine was not created !", 0, this);
			octave = null;
		}
	}
	...
	public void runCode(String code)
	{
		...
		try
		{
			...
			for(int i = 0; i < breaks.length; i++)
			{
				...
				if(i == 0 && breaks[i].equals("")) curLine--;
			}
		}
		catch(OctaveEvalException e)
		{
			JEXStatics.logManager.log(e.getMessage(), 0, this);
	...
	{
		if (octave == null){
			JEXStatics.logManager.log("Error ! Octave engine is null cannot connect terminal !", 0, this);
		}
		else {
