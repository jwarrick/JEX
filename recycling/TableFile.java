...
public class TableFile {
	...
	{
		// Create the dimTable and record the row where data begins (first row in file is row 0).
		...
		try
		{
			...
			this.header.add(JEXTableWriter2.VALUE);
			csvReader.close();
		}
	...
	/**
	 * Get a subset of the data within this file to keep memory usage low.
	 * The DimensionMap filter should not be null. If you want the whole table
	 * then use a new empty dimension map (i.e. new DimensionMap()) or use the
	 * TableFile.getTable() method with no filter argument.
	 * @param filter
	 * @return
	 */
	...
	public Table getData(DimensionMap filter)
	{
		...
		{
			final CSVReader csvReader = new CSVReader(new FileReader(path));
			while (!csvReader.isEOF() && fileRowCount < firstDataRow)
			{
				System.out.println(csvReader.readLine());
				fileRowCount ++;
			}
			for(DimensionMap map : this.dimTable.getIterator(filter))
			{
				...
				while(!csvReader.isEOF() && !foundMatch)
				{
					csvReader.readFields(fields);
//					 System.out.println(fields.toString());
					DimensionMap rowMap = getMap(header, fields);
	...
	public static void saveTable(String filePath, DimTable dimTable, Map<DimensionMap,String> data)
	{
		try
		{
			...
			// Write the data
			for (DimensionMap map : dimTable.getIterator())
			{
				...
				if(JEXStatics.statusBar != null)
				{
					if(percent != (int)percentage) // Only update the status bar when necessary, otherwise if called too frequently, it never updates
					{
	...
	public static CSVWriter startWritingTable(String filePath, DimTable dimTable)
	{
		try
		{
			...
			csvWriter.writeField(JEXTableWriter2.VALUE);
			csvWriter.newLine();
			return csvWriter;
		}
		catch (final IOException e)
	...
	public static PrintWriter startWritingArrfFile(String filePath, Instances instances)
	{
		try
		{
...
//	public static void writeArffRow(PrintWriter writer, Instances intances, DimensionMap map, String value)
//	{
//		writer.println(inst);
//        incrementalCounter++;
