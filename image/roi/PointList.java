	...
	public PointList(String polygonPts)
	{
		...
		if(polygonPts != null)
		{
			...
			while(itr.hasNext())
			{
				...
				if (pt.get(0).equals("") || pt.get(1).equals("")) continue;
...
//	}
	 ...
	 * around the specified point (i.e. radius up, radius down,
	 * radius left, and radius right). Returns the nearest point
	 * within that range.
	 * @param p
	 * @param radius
	 * @return
	...
	public IdPoint nearestPointInRectRange(Point p, double radius)
	{
		...
		if(Math.abs(p.x-retP.x) <= radius && Math.abs(p.y-retP.y) <= radius) return retP;
		else return null;
	}
	
	/**
	 * radius is actually a circular region extending radius pixels
	 * around the specified point (i.e. radius up, radius down,
	 * radius left, and radius right). Returns the nearest point
	 * within that range.
	 * @param p
	...
	public boolean equals(PointList l)
	{
		...
		if(this.getBounds().equals(l.getBounds()))
		{
			for(int i = 0; i < this.size(); i++)
			{
				if(!this.get(i).equals(l.get(i))) return false;
			}
		}
		return true;
	}
