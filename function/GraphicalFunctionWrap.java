...
public class GraphicalFunctionWrap extends JDialog implements ActionListener{
	...
	HashMap<Integer,JParameterEntryPanel[]> paramPanelForStep;
	...
	public GraphicalFunctionWrap(GraphicalCrunchingEnabling function, HashMap<String,Parameter> parameters){
		...
		paramPanelForStep  = new HashMap<Integer,JParameterEntryPanel[]>();
	}
	...
	/**
	 * Add a step to the function 
	 * give the step a naming description
	 * and a list of parameters it requires
	 * @param index
	 * @param name
	 * @param parameters
	 */
	...
	public void initialize(){
		JEXStatics.logManager.log("Initializing the graphics",1,this);
		...
		for (Integer key: keys){
			...
			JParameterEntryPanel[] panels = new JParameterEntryPanel[paramNames.length];
			for (int i=0, len=paramNames.length; i<len; i++){
				Parameter p = parameters.get(paramNames[i]);
				if (p == null) continue;
				panels[i] = new JParameterEntryPanel(p);
			}
...
//		int displayUntil = (currentStep >= labelForStep.size()) ? labelForStep.size()-1 : currentStep;
		
		int step = function.getStep();
		JEXStatics.logManager.log("Displaying until step "+step,1,this);
		for (int i=0; i<=step; i++){
			JLabel label = jlabelForStep.get(new Integer(i));
			JButton button = buttonForStep.get(new Integer(i));
			JParameterEntryPanel[] panels = paramPanelForStep.get(new Integer(i));
			
			StepPanel paramPanel = new StepPanel(label.getText(),panels,button);
			if (i<step && autoReduce) paramPanel.reduce();
	...
	public boolean start(){
		...
		if (this.displayLoopPanel) contentPane.add(buttonPane,BorderLayout.PAGE_END);

		JEXStatics.logManager.log("Starting the modal interface",1,this);
		this.setModal(true);
	...
	public void validateParameters(){
		...
		for (Integer index: indeces){
			JParameterEntryPanel[] panels = paramPanelForStep.get(index);
			if (panels == null) continue;
			for (JParameterEntryPanel panel: panels){
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == validateButton){
			JEXStatics.logManager.log("Validating the function... end of visual processing",1,this);
			function.finishIT();
			isValid = true;
			this.dispose();
...
//			this.refresh();
			...
			for (Integer key: keys){
				if (e.getSource() == buttonForStep.get(key)){
					JEXStatics.logManager.log("Finished step "+key+" going to next step",1,this);
					validateParameters();
	...
	class StepPanel extends JRoundedCollapsablePanel implements ActionListener {
		...
		JParameterEntryPanel[] parameters ;
		JButton doneButton ;
		String  label;
		                     
		StepPanel(String label, JParameterEntryPanel[] parameters, JButton button){
			super();
			this.parameters = parameters;
			this.label      = label;
		...
		private void rebuild(){
			...
			if (parameters != null){
				for (JParameterEntryPanel panel: parameters){
					if (panel == null) continue;
