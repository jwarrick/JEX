...
public class MaximumFinder {
    ...
    /** output type names */
    public final static String[] outputTypeNames = new String[]
        {"Single Points", "Maxima Within Tolerance", "Segmented Particles", "Roi", "Count"};
     ...
     *                       If ROI or COUNT is selected for the output type a RoiPlus or Integer is returned.
     */
    public Object findMaxima(ImageProcessor ip, double tolerance, double threshold, int outputType, boolean excludeOnEdges, boolean isEDM, Roi roiArg, boolean lightBackground)
    {
        ...
        Rectangle roi;
        if(roiArg == null) roi = new Rectangle(0, 0, ip.getWidth(), ip.getHeight());
        else roi = roiArg.getBounds();
        ...
        PointList points = analyzeAndMarkMaxima(ip, typeP, maxPoints, excludeOnEdges, excludeEdgesNow, isEDM, globalMin, tolerance, outputType, maxSortingError);
        ...
        if(outputType == SEGMENTED)
        {
            ...
            //if (false) // change to true to see the max points in the segmented image.
            //{
...
PointList analyzeAndMarkMaxima(ImageProcessor ip, ByteProcessor typeP, long[] maxPoints, boolean excludeOnEdges, boolean excludeEdgesNow, boolean isEDM, float globalMin, double tolerance, int outputType, float maxSortingError) {
   ...
   /** Get estimated "true" height of a maximum or saddle point of a Euclidian Distance Map.
    ...
    void deleteParticle(int x, int y, ByteProcessor ip, Wand wand) {
        ...
        if (wand.npoints==0) {
            JEXStatics.logManager.log("wand error selecting edge particle at x, y = "+x+", "+y, 0, this);
            return;
        }
            ...
            if (remaining>0 && level>1) {   // any pixels that we have not reached?
                ...
                if (nextLevel > 0) {
                    ...
                    for (int i=0, p=levelStart[level]; i<remaining; i++, p++) {
                        ...
                        if ((pixels[pOffset]&255)==255) JEXStatics.logManager.log("ERROR", 0, this);
