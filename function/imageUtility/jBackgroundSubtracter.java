...
Version 10-Jan-2008
*/
public class jBackgroundSubtracter implements ExtendedPlugInFilter, DialogListener {
    /* parameters from the dialog: */
    public static double radius = 50;  // default rolling ball radius
    public static boolean lightBackground = Prefs.get("bs.background", true);
    private static boolean separateColors; // whether to create a separate background for each color channel
    public static boolean createBackground;   // don't subtract background (e.g., for processing the background before subtracting)
    public static boolean useParaboloid; // use "Sliding Paraboloid" instead of rolling ball algorithm
    public static boolean doPresmooth = true; // smoothen the image before creating the background
    ...
    private int pass;
    public int flags = DOES_ALL|FINAL_PROCESSING|KEEP_PREVIEW|PARALLELIZE_STACKS;
    ...
    public void run(ImageProcessor ip) {
    	if(!createBackground) ip.setSnapshotPixels(ip.getPixels());
    ...
    public void rollingBallBackground(ImageProcessor ip, double radius, boolean createBackground,
        ...
        for (int channelNumber=0; channelNumber<ip.getNChannels(); channelNumber++) {
			...
			if(!createBackground) orig = (float[]) fp.getPixelsCopy();
            //ImagePlus jay2 = new ImagePlus("snapshot", ip);
            //jay2.show();
            if (useParaboloid)
            ...
            if (createBackground)
                ...
                if (ip instanceof FloatProcessor) {         //here ip and fp are the same (bgPixels will be output)
                    ...
                    //ImagePlus jay = new ImagePlus("duh", ip);
                	//jay.show();
                    for (int p=0; p<bgPixels.length; p++)
                        bgPixels[p] = orig[p]-bgPixels[p];
