 ...
 *
 */
public class JEX_ScanSlideImportData extends ExperimentalDataCrunch{
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(IMAGE,"Image Set");
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		TreeMap<String,TreeMap<String,TreeMap<String,TreeMap<String,File>>>> posMap = getFileMap(imageFiles);
		...
		int[] rcs = getNumRowsAndCols(numLocs);
		int rows = rcs[0];
		int cols = rcs[1];
		...
		for(String locKey : locMap.keySet())
		{
			...
			JEXStatics.logManager.log("Position: " + locKey + "RC: " + rc[0] + "," + rc[1], 0, this);
			rc = getNextRCPosition(numLocs, rc[0], rc[1]);
		}
	...
	{
		int[] rcs = getNumRowsAndCols(_RXC);
		int rows = rcs[0];
		int cols = rcs[1];
		
		// Use this code if the order snakes throught he slide
//		boolean isOddCol = (curCol % 2 == 1);
//		boolean isLastInCol = ((isOddCol && curRow == rows) || (!isOddCol && curRow == 1));
//		int nextCol = curCol;
//		int nextRow = curRow;
//		if(isLastInCol)
//		{
