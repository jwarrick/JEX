 ...
 *
 */
public class JEX_ChangeFileName extends ExperimentalDataCrunch{
...
//		parameterArray.addParameter(p7);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		for(File file : files)
		{
			...
			if(fileName.length() <= n + m) continue;
			...
			if(oldMiddle.equals("*") || oldMiddle.equals(subName))
			{
				...
				if(!(new File(newPath)).exists()) this.renameFile(file, newPath);
			}
			count = count + 1;
	...
	private void renameFile(File toBeRenamed, String toFile)
	{
        if (!toBeRenamed.exists() || toBeRenamed.isDirectory()) {
            ...
            JEXStatics.logManager.log("Can't rename. File does not exist: " + toBeRenamed, 0, this);
            return;
        }
        ...
        //Rename
        if (!toBeRenamed.renameTo(newFile)) JEXStatics.logManager.log("Error renmaing file" + toBeRenamed + " to " + newFile, 0, this);
    }
}
