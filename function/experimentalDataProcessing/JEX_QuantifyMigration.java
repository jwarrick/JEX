 ...
 *
 */
public class JEX_QuantifyMigration extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p3 = new Parameter("Auto-thresholding","Algorithm for auto-thresholding",FormLine.DROPDOWN,new String[] {"Minimum","MaxEntropy","Otsu","Moments","Li","Default","Mean","Huang","Triangle","MinError(I)","Percentile"},8);
		Parameter p4 = new Parameter("Threshold","Overide auto-thresold if value is greater than 1","-1");
		Parameter p5 = new Parameter("Watershed","Do a watershedding to split cells close together",FormLine.DROPDOWN,new String[] {"true","false"},1);
		...
		Parameter p8 = new Parameter("Horizontal","Migration channel is horizontal?",FormLine.DROPDOWN,new String[] {"true","false"},0);
		Parameter p9 = new Parameter("Automatic","Enable visual interface",FormLine.DROPDOWN,new String[] {"true","false"},1);
...
class FindNeutrophilsHelperFunction implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	private void displayImage(int index){
		...
		JEXStatics.logManager.log("Placing in imagepanel "+images.get(index),1,this);
...
//			FunctionUtility.imSave(bimp, "./quantifyMigration1.tif");
		...
		else 
		{
			...
			jBackgroundSubtracter.createBackground = false;
			jBackgroundSubtracter.useParaboloid = false; // use "Sliding Paraboloid" instead of rolling ball algorithm
			jBackgroundSubtracter.doPresmooth = false;
			bS.run(imp);
...
//			FunctionUtility.imSave(imp, "./quantifyMigration1.tif");
	...
	private void threshold(){
		...
		// If the thresold value is more than 1 then use the manually entered value
		if (threshold < 0)
		{
			JEXStatics.logManager.log("Thresholding using "+threshMethod+" algorithm", 1, this);
		...
		{
			JEXStatics.logManager.log("Thresholding using manually entered value "+threshold, 1, this);
		}
		...
		// Watershed
//		Boolean watershed = Boolean.parseBoolean(params.getValueOfParameter("Watershed"));
		if (watershed)
		{
			EDM edm = new EDM();
			edm.toWatershed(imp);
