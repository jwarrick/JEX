 ...
 *
 */
public class JEX_TrackCells extends ExperimentalDataCrunch {
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p6 = new Parameter("Extension algorithm","Use a different extention algorithm",FormLine.DROPDOWN,new String[] {"Minimize displacements","Closest neighbour"},1);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		if (!data1.getTypeName().getType().equals(JEXData.ROI)) return false;
...
class TrackHelperFunction implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	public void runStep(int index) {
		...
		if (auto && index < rois.size()-1) loopNext();
	}
	public void runNext(){}
	...
	public void loopNext(){
		index = index + 1;
		if (index >= rois.size()-1) index = rois.size()-1;
		runStep(index);
	}
	...
	public void finishIT() {
		JEXStatics.logManager.log("Retrieving and saving the tracks", 1, this);
