 ...
 *
 */
public class JEX_WholeBloodNeutrophilMigration extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p2 = new Parameter("Preset cell","Use a preset cell instead of custom ROI",FormLine.DROPDOWN,new String[] {"true","false"},1);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		if (data3 != null && (!data3.getTypeName().getType().equals(JEXData.IMAGE))) return false;
...
class NeutrophilTrackerHelperFunction2 implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	NeutrophilTrackerHelperFunction2(
			JEXData imset, 
			JEXData roiData, 
			JEXData cell, 
			JEXEntry entry, 
...
//			cellImage = ImageReader.readObjectToImagePlus(cell);
	...
	private void makeROI(){
		if (roiData != null)
		{
			...
			// Get the rectangle
			Rectangle r = (roip!=null && roip.roi!=null) ? roip.roi.getBounds() : null;
...
//		ImageProcessor imp2 = maskImage(imp,rect);
	...
	private void track(){
		...
		for (Integer key: keys){
			...
			JEXStatics.logManager.log("Extended points for frame " + key + " of " + keys.size() + ".",1,this);
...
//		TrackStatistics stats = new TrackStatistics(trajSet);
		...
		{
			// Get the number of trajectories currently active between times j and j+interval
			...
			for (int i=0, len=finaltrajs.length; i<len; i++)
			{
				...
				for (int z=0; z<interval; z++)
				{
					...
					if (p!=null) numberOfTracks = numberOfTracks + 1;
				}
			}
