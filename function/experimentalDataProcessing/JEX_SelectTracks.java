 ...
 *
 */
public class JEX_SelectTracks extends ExperimentalDataCrunch {
	...
	public ParameterSet requiredParameters() {
		Parameter p1 = new Parameter("Automatic","Perform without displaying GUI interface",FormLine.DROPDOWN,new String[] {"true","false"},1);
		...
		Parameter p6 = new Parameter("Percent","Select a percent of the tracks instead of a hard number",FormLine.DROPDOWN,new String[] {"true","false"},1);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		if (trajectories == null) return false;
		
		// Make xml element
		List<Trajectory> selected = new ArrayList<Trajectory>(0);
		for (Trajectory traj: trajectories){
			if (traj.nbPoints() > 1) selected.add(traj);
		}
...
class TrajectoryComparator implements Comparator<Trajectory> {
	...
	public int compare(Trajectory trajOne, Trajectory trajTwo) {
		if (mode == LENGTH){
			if (trajOne.nbPoints() > trajTwo.nbPoints()) return -1;
			if (trajOne.nbPoints() == trajTwo.nbPoints()) return 0;
			return 1;
		}
		...
		if (mode == VELOCITY){
			if (statOne.meanVelocity > statTwo.meanVelocity) return -1;
			if (statOne.meanVelocity == statTwo.meanVelocity) return 0;
			return 1;
		}
		else if (mode == MOSTDIR){
			if (statOne.CI > statTwo.CI) return -1;
			if (statOne.CI == statTwo.CI) return 0;
			return 1;
		}
