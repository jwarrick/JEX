 ...
 *
 */
public class JEX_StackProjection extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p1 = new Parameter("Math Operation","Type of math operation to perform.", FormLine.DROPDOWN,new String[]{"mean","max","min","sum","std. dev.","median"},5);
		Parameter p2 = new Parameter("Sliding Window Projection","Perform the projection for the whole stack or for N number of images at a time, shifting by 1 each time.",FormLine.DROPDOWN,new String[] {"true","false"},1);
		...
		for(DimensionMap map : maps)
		{
			if(slidingWindow)
			{
				for(int i = 0; i <= dimToProject.size()-slidingWindowSize; i++)
				{
					...
					FloatProcessor imp = this.evaluate(stackToProject, mathOperation, stackMaps.size());
