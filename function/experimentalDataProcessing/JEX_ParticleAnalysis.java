...
import function.imageUtility.ParticleAnalyzer;
 ...
 *
 */
public class JEX_ParticleAnalysis extends ExperimentalDataCrunch{
	...
	public String getInfo() {
		String result = "Analyze particles in a binary image or use the binary image drive analysis of a grayscale image.";
		return result;
	}
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(IMAGE,"Outline Image");
		defaultOutputNames[1] = new TypeName(FILE,"ROI File");
		defaultOutputNames[2] = new TypeName(FILE,"CSV Data");
...
//		Parameter p0 = new Parameter("Dummy Parameter","Lets user know that the function has been selected.",FormLine.DROPDOWN,new String[] {"true"},0);
		Parameter p0 = new Parameter("Particles Are White?","Are the particles white on a black background?",FormLine.DROPDOWN, new String[] {"True", "False"}, 0);
		...
		Parameter p5 = new Parameter("Exclude On Edges?","Exclude particles that touch the edge of the image?",FormLine.DROPDOWN,new String[] {"True","False"}, 0);
		Parameter p6 = new Parameter("Fill Holes?","Fill holes inside particles?",FormLine.DROPDOWN,new String[] {"True","False"}, 0);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if (binaryData == null || !binaryData.getTypeName().getType().equals(JEXData.IMAGE)) return false;
		...
		if (grayData != null && !grayData.getTypeName().getType().equals(JEXData.IMAGE)) return false;
		
		// Gather parameters
		boolean particlesAreWhite = Boolean.parseBoolean(parameters.getValueOfParameter("Particles Are White?"));
		double minSize = Double.parseDouble(parameters.getValueOfParameter("Min Size"));
		double maxSize = Double.parseDouble(parameters.getValueOfParameter("Max Size"));
		double minCirc = Double.parseDouble(parameters.getValueOfParameter("Min Circularity"));
		double maxCirc = Double.parseDouble(parameters.getValueOfParameter("Max Circularity"));
		boolean excludeOnEdges = Boolean.parseBoolean(parameters.getValueOfParameter("Exclude On Edges?"));
		boolean fillHoles = Boolean.parseBoolean(parameters.getValueOfParameter("Fill Holes?"));
		
		// Prepare output maps
		TreeMap<DimensionMap,String> outlineMap = new TreeMap<DimensionMap,String>(); // defaultOutputNames[0] = "Outline Image";
		TreeMap<DimensionMap,String> outputRoiMap = new TreeMap<DimensionMap,String>(); // defaultOutputNames[1] = "ROI File";
		TreeMap<DimensionMap,String> csvMap = new TreeMap<DimensionMap,String>(); // defaultOutputNames[2] = "CSV Data";
		...
		HashMap<ROIPlus,ImageStatistics> results;
		for(DimensionMap map : binaryMap.keySet())
		{
			...
			ParticleAnalyzer pa = ParticleAnalyzer.analyzeParticles(gray, binary, particlesAreWhite, excludeOnEdges, fillHoles, minSize, maxSize, minCirc, maxCirc, ParticleAnalyzer.DEFAULT_MEASUREMENTS);
			results = pa.getMeasurements();
			outline = pa.getOutlineImage();
			String[] outputPaths = saveResults(results);
		...
		JEXData output1 = ImageWriter.makeImageStackFromPaths(outputNames[0].getName(), outlineMap);
		JEXData output2 = FileWriter.makeFileTable(JEXData.FILE, outputNames[1].getName(), outputRoiMap);
		JEXData output3 = FileWriter.makeFileTable(JEXData.FILE, outputNames[2].getName(), csvMap);
	...
	private String[] saveResults(HashMap<ROIPlus,ImageStatistics> results)
	{
