 ...
 *
 */
public class JEX_MeasureFluorescentIntensityWithMask extends ExperimentalDataCrunch{
	...
	public TypeName[] getOutputs() {
		...
		defaultOutputNames[2] = new TypeName(IMAGE,"Masked image");
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		for (DimensionMap dim:images.keySet()){
			...
			ByteProcessor bimpmask = (ByteProcessor) imask.getProcessor().convertToByte(true); // should be a float processor
		...
		JEXData outputData1 = ValueWriter.makeValueTable(outputNames[0].getName(), outputMeanMap); 
		outputData1.setDataObjectInfo("Mean value of the image under the mask area");
		realOutputs.add(outputData1);
		
		// Save the area data
		JEXData outputData2 = ValueWriter.makeValueTable(outputNames[1].getName(), outputAreaMap);
		outputData2.setDataObjectInfo("Area of the mask");
		realOutputs.add(outputData2);
		
		// Save the area data
		JEXData outputData3 = ImageWriter.makeImageStackFromImagePluses(outputNames[2].getName(), outputImageMap);
		outputData3.setDataObjectInfo("Combination of the mask and the image to analyze");
		realOutputs.add(outputData3);
