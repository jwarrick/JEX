 ...
 *
 */
public class JEX_LocalMovementAnalysis extends ExperimentalDataCrunch{
...
//			imp.drawLine(w, 0, w, imp.getHeight());
			if (w+grid < imp.getWidth()) xPos.add(w);
			w = w + grid;
		}
...
//			imp.drawLine(0, h, imp.getWidth(), h);
			if (h+grid < imp.getHeight()) yPos.add(h);
			h = h + grid;
		}
		
		JEXStatics.logManager.log("X and Y positions found", 1, this);
	}
...
//			yPos.add(h);
	...
	private void findDeformation(){
		...
		String csvArrayX = ",";
		String csvArrayY = ",";
		for (int x: xPos) csvArrayX = csvArrayX + x + ",";
		for (int x: xPos) csvArrayY = csvArrayY + x + ",";
		csvArrayX = csvArrayX + "\n";
		csvArrayY = csvArrayY + "\n";
		for (int y: yPos){
			...
			for (int x: xPos){
//				JEXStatics.logManager.log("Finding deformation at position "+x+" - "+y, 1, this);
...
//				JEXStatics.logManager.log("   Displacement minimum for vector "+maxScore.dX+" - "+maxScore.dY, 1, this);
...
//			String newFileName = FunctionUtility.getNextName(localDir, f.getName(), "Disp");
...
//			String newFileName2 = FunctionUtility.getNextName(localDir, "Matrix.csv", "Disp");
	...
	public float scoreConvolution(float[][] image, float[][] convolveImage, int posX, int posY, float mean){
		...
		for (int i=0,leni=convolveImage.length;i<leni;i++){
			for (int j=0,lenj=convolveImage[0].length;j<lenj;j++){
				...
				if (x<0)                continue;
				if (x>=image.length)    continue;
				if (y<0)                continue;
				if (y>=image[0].length) continue;
...
//				JEXStatics.logManager.log("ooo Getting score at position "+x+" - "+y, 1, this);
