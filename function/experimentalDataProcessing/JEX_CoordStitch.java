 ...
 *
 */
public class JEX_CoordStitch extends ExperimentalDataCrunch{
	
	public JEX_CoordStitch(){}
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p1 = new Parameter("Output Bit Depth","Bit depth to save the image as.",FormLine.DROPDOWN, new String[]{"8","16"},1);
		Parameter p2 = new Parameter("Normalize Intensities Fit Bit Depth","Scale intensities to go from 0 to max value determined by new bit depth (\'true\' overrides intensity multiplier).",FormLine.DROPDOWN,new String[]{"true","false"},1);
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if (valueData == null || !valueData.getTypeName().getType().equals(JEXData.VALUE)) return false;
		...
		if(stitchedFile == null) return false;
		
		// Set the outputs
		realOutputs.add(ImageWriter.makeImageObject(outputNames[0].getName(), stitchedFile.getAbsolutePath()));
	...
	{		
		...
		while(itr.hasNext() && itrXY.hasNext())
		{
			...
			JEXStatics.logManager.log("Getting file " + ImageReader.readObjectToImagePath(imageData, map) + " in entry " + entry.getTrayX() + "," + entry.getTrayY() + " for dim " + map.toString(), 0, "JEX_CoordStitch");
	...
	private PointList convertStageMovements(File stageMovements, int dxImage, int dyImage, int imageIndex, double scale)
	{
		...
		while(itr.hasNext())
		{
			...
			xt = (temp.x);
			yt = (temp.y);
			theta = Math.atan2(yt,xt) + dtheta; // Multiply dy by -1 to account for reversed coordinates of microscope stage.
