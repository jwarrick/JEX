 ...
 *
 */
public class JEX_FibrinogenFluorescenceQuantification extends ExperimentalDataCrunch{
	...
	public String getName() {
		String result = "Fibronectin quantification";
		return result;
	}
	...
	public String getInfo() {
		String result = "Determine fluorescent intensity from fibronectin fibrils";
		return result;
	}
	...
	public TypeName[] getInputNames(){
		...
		inputNames[0] = new TypeName(IMAGE,"Fibrinogen Image");
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(VALUE,"Integrated intensity");
		defaultOutputNames[1] = new TypeName(VALUE,"Mean intensity");
		defaultOutputNames[2] = new TypeName(VALUE,"Surface coverage");
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		// Run the function
		FibrinogenFluorescenceCounter graphFunc = new FibrinogenFluorescenceCounter(entry,data1,outputNames,parameters);
		graphFunc.doit();
		JEXData output1 = graphFunc.integratedIntensity;
		JEXData output2 = graphFunc.meanIntensity;
		JEXData output3 = graphFunc.surfaceCoverage;
		JEXData output4 = graphFunc.maskImage;
...
class FibrinogenFluorescenceCounter implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	// Outputs
	public JEXData integratedIntensity;
	...
	private void analyze(){		
		...
		meanDens = stats.mean;
		intDens  = stats.mean * surfCov;
		
		integratedIntensity = ValueWriter.makeValueObject(outputNames[0].getName(), ""+intDens);
		integratedIntensity.setDataObjectInfo("Integrated density of the fibrinogen fibrils");
		
		meanIntensity = ValueWriter.makeValueObject(outputNames[1].getName(), ""+meanDens);
		meanIntensity.setDataObjectInfo("Mean density of the fibrinogen fibrils");
		
		surfaceCoverage = ValueWriter.makeValueObject(outputNames[2].getName(), ""+surfCov);
		surfaceCoverage.setDataObjectInfo("Surface coverage of the fibrinogen fibrils");
