...
public class JEX_OverlayImageSets extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		Parameter p1 = new Parameter("Channel 1 color","Color of channel 1",FormLine.DROPDOWN,new String[] {"R","G","B"},0);
		Parameter p2 = new Parameter("Channel 2 color","Color of channel 2",FormLine.DROPDOWN,new String[] {"R","G","B"},1);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		for (DimensionMap dim:images1.keySet()){
			...
			ij.process.ByteProcessor imp1 = (ij.process.ByteProcessor) im1.getProcessor().convertToByte(true); // should be a float processor
			ij.process.ByteProcessor imp2 = (ij.process.ByteProcessor) im2.getProcessor().convertToByte(true); // should be a float processor
			
			////// Begin Actual Function
			int w=im1.getWidth(), h=im1.getHeight();
			byte[] r=null, g=null, b=null;
			ColorProcessor cp = new ColorProcessor(w, h);
			if(color1 == 0) r = (byte[]) imp1.getPixels();
			if(color2 == 0) r = (byte[]) imp2.getPixels();
			if(r == null) r = new byte[w*h];
			if(color1 == 1) g = (byte[]) imp1.getPixels();
			if(color2 == 1) g = (byte[]) imp2.getPixels();
			if(g == null) g = new byte[w*h];
			if(color1 == 2) b = (byte[]) imp1.getPixels();
			if(color2 == 2) b = (byte[]) imp2.getPixels();
			if(b == null) b = new byte[w*h];
			cp.setRGB(r, g, b);
			////// End Actual Function
