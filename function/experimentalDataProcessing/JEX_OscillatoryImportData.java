...
import utilities.SimpleFileFilter;
 ...
 *
 */
public class JEX_OscillatoryImportData extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		Parameter p0 = new Parameter("Folder Path","Folder in which to find Data","/Volumes/IX81_Files/Jay/");
		Parameter p1 = new Parameter("Multi-Folder Import","Try to parse each folder within this folder into each entry",FormLine.DROPDOWN,new String[]{"true","false"},1);
//		Parameter p2 = new Parameter("Outlier p-value","Statistical significance for determining an outlier","0.05");
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		JEXStatics.logManager.log("Importing data from folder: " + folder.getPath(), 1, this);
		if(folder == null || !folder.exists() || !folder.isDirectory())
		{
			JEXStatics.statusBar.setStatusText("Error reading path: " + folder.getPath());
			return false;
		}
		boolean folderMode = Boolean.parseBoolean(parameters.getValueOfParameter("Multi-Folder Mode"));
//		double alpha = Double.parseDouble(parameters.getValueOfParameter("Outlier p-value"));
		...
		else
		{
			...
			if(logFiles.size() == 0) return false;
	...
	private List<List<File>> getSeparatedFileLists(List<File> imageFiles)
	{
		...
		for(String pre : filePrefixes)
		{
			...
			{
				JEXStatics.logManager.log("Couldn't find matching prefix", 0, this);
			}
			else
			{
