 ...
 *
 */
public class JEX_CTCMLsGivenCalc extends ExperimentalDataCrunch{
...
//		parameterArray.addParameter(p7);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if(!epi.getTypeName().getType().equals(JEXData.VALUE) ) return false;

		//GFP Counts
		JEXData seed = inputs.get("uL seeded");
		if(!seed.getTypeName().getType().equals(JEXData.VALUE) ) return false;
		
		//mL Whole Blood Equivalents
		JEXData mrna = inputs.get("uL for mRNA");
		if(!mrna.getTypeName().getType().equals(JEXData.VALUE) ) return false;
		
		//mL whole blood to start
		JEXData equiv = inputs.get("mL equivalent seeded");
		if(equiv == null || !equiv.getTypeName().getType().equals(JEXData.VALUE) ) return false;
