 ...
 *
 */
public class JEX_AnalyzePodosomes extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p6 = new Parameter("Recalculate","Recalculate",FormLine.DROPDOWN,new String[] {"true","false"},1);
...
class FindPodosomeHelperFunction implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	private void subtractBackground(){
		...
		if (roi != null){
			java.awt.Rectangle rect = roi.roi.getBounds();
			imp.setRoi(rect);
...
//		imp.invert();
	...
	private void makeTracks(){
		...
		for (int i=0, len=pointListArray.length; i<len; i++){
			...
			JEXStatics.logManager.log("Extended points for frame " + i + " of " + len + ".",1,this);
	...
	private void selectTracks(){
		...
		for (Trajectory traj: finaltrajs){
			if (traj.nbPoints() > 1) trajSet.addTrajectory(traj);
		}
