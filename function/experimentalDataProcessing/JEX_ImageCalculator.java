 ...
 *
 */
public class JEX_ImageCalculator extends ExperimentalDataCrunch{
...
//		Parameter p0 = new Parameter("Dummy Parameter","Lets user know that the function has been selected.",FormLine.DROPDOWN,new String[] {"true"},0);
		Parameter p1 = new Parameter("Math Operation","Math operation to perform using the pixel information from Image A and B.",FormLine.DROPDOWN, new String[]{"A+B","A-B","A*B","A/B","|A-B|","MAX","MIN","AVERAGE","AND","OR","XOR","COPY","COPY Transparent 0"},4);
		Parameter p2 = new Parameter("Output Bit-Depth","Bit-Depth of the output image",FormLine.DROPDOWN,new String[]{"8","16","32"},2);
...
//		parameterArray.addParameter(p0);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if (imageAData == null || !imageAData.getTypeName().getType().equals(JEXData.IMAGE)) return false;
		// Collect the inputs
		JEXData imageBData = inputs.get("Image B");
		if (imageBData == null || !imageBData.getTypeName().getType().equals(JEXData.IMAGE)) return false;
		...
		if(method.equals("A+B")) methodInt = Blitter.ADD;
		else if(method.equals("AND")) methodInt = Blitter.AND;
		else if(method.equals("AVERAGE")) methodInt = Blitter.AVERAGE;
		else if(method.equals("COPY")) methodInt = Blitter.COPY;
		else if(method.equals("COPY_ZERO_TRANSPARENT")) methodInt = Blitter.COPY_ZERO_TRANSPARENT;
		else if(method.equals("|A-B|")) methodInt = Blitter.DIFFERENCE;
		else if(method.equals("A/B")) methodInt = Blitter.DIVIDE;
		else if(method.equals("MAX")) methodInt = Blitter.MAX;
		else if(method.equals("MIN")) methodInt = Blitter.MIN;
		else if(method.equals("A*B")) methodInt = Blitter.MULTIPLY;
		else if(method.equals("OR")) methodInt = Blitter.OR;
		else if(method.equals("A-B")) methodInt = Blitter.SUBTRACT;
		else if(method.equals("XOR")) methodInt = Blitter.XOR;
