 ...
 *
 */
public class JEX_CountParticlesHaemocytometer extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		...
		//Parameter p2 = new Parameter("Threshold","Threshold for identifying particle locations","50.0");
		Parameter p2 = new Parameter("Auto-thresholding","Algorithm for auto-thresholding",FormLine.DROPDOWN,new String[] {"Minimum","MaxEntropy","Otsu","Moments","Li","Default","Mean","Huang","Triangle","MinError(I)","Percentile"},8);
...
class HaemocytometerCountingHelperFunction implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	HaemocytometerCountingHelperFunction(
			JEXData imset, 
			JEXData roiData, 
			JEXEntry entry, 
			TypeName[] outputNames, 
...
//		imagepanel.setPointListArray(null,null);
	...
	public void loopNext(){
		...
		if (lowerKey != null) currentDim = lowerKey;
		currentRoi = roiMap.get(currentDim);
		im         = new ImagePlus(imageMap.get(currentDim));
		this.displayImage(im, currentRoi, currentPList);
	...
	public void loopPrevious(){
		...
		if (higherKey != null)  currentDim = higherKey;
		currentRoi = roiMap.get(currentDim);
		im         = new ImagePlus(imageMap.get(currentDim));
		this.displayImage(im, currentRoi, currentPList);
...
//		findParticles();
	...
	private void convolve()
	{
		...
		for (DimensionMap dim: imageMap.keySet())
		{
			...
			if (image==null || image.getProcessor()==null) continue;
...
//			String method = "MaxEntropy";
	...
	private void findParticles()
	{
		...
		for (DimensionMap dim: imageMap.keySet())
		{
			...
			if (image==null || imp==null || roip == null) continue;

			// Use ImageJ Particle Analyzer on data1
			int options = 0;
			int measure = Measurements.AREA | Measurements.CIRCULARITY  | Measurements.INTEGRATED_DENSITY
			| Measurements.CENTROID | Measurements.ELLIPSE;
			...
			Rectangle rect = roip.roi.getBounds();
			for (int i=0; i<xPos.length; i++)
			{
				...
				if (rect.contains(px, py)) result.add(p);
			}
	...
	private void analyze()
	{
		...
		{	
			...
			if (image == null || pList == null) continue;
	...
	public void pressedPoint(Point p) {
		if (interactionMode){
			JEXStatics.logManager.log("Extract cell image", 1, this);
			extractImage(p);
		}
	}
