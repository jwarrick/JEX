...
public class JEX_WholeBloodNeutrophilMigration2 extends ExperimentalDataCrunch{
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		if (data2 != null && (!data2.getTypeName().getType().equals(JEXData.ROI))) return false;
...
//				if (index+i < jimages.size()) imagesToaverage.add(jimages.get(index+i).getImagePlus());
//			}
//			
//			// If not enough images were found then grab some before the image of interest
...
//				JEXStatics.logManager.log("Added image to average",1,this);
//			}
//			
...
//				if (i < jimages.size()) imagesToaverage.add(jimages.get(i).getImagePlus());
				if (i < jimages.size()) imagesToaverage.add(new ImagePlus(jimages.get(i)));
			}
...
//		JEXStatics.logManager.log("Convolving image",1,this);
//		
//		// make cell image
//		cellImage = makeCellOfRadius(4*psize,4*psize,psize);
//		
//		// Make bufferedimage source
//		BufferedImage bim = ImageUtility.imagePlus2BufferedImage(imageWithoutImmobile);
//		
//		// make the kernel
//		FloatProcessor convFloat = (FloatProcessor) cellImage.getProcessor().convertToFloat();
//		float[][] convMat = convFloat.getFloatArray();
//		Kernel kernel = SimpleConvolve.makeConvolutionKernel(convMat, cellImage, imageWithoutImmobile);
//		
...
//		imp = (ByteProcessor) convolvedImage.getProcessor().convertToByte(true);
...
//		ImagePlus im = jimages.get(index).getImagePlus();
		...
		JEXStatics.logManager.log("Finding particles",1,this);
...
//		PointList particles = finder.findMaximum(convolvedImage, new Rectangle(0,0,convolvedImage.getWidth(),convolvedImage.getHeight()));
//		
...
//			// If p is outside of the inner ROI then remove it as a starting point for a track
...
//			pointOutside         = pointOutside || (p.getX() > convolvedImage.getWidth() - innerRadius);
//			pointOutside         = pointOutside || (p.getY() > convolvedImage.getHeight()- innerRadius);
...
//		}
	...
	private PointList locateSeedParticles(ImagePlus image, int index, PointList existingPoints){
		...
		for (Point p: particles){
			...
			// If p is close to an existing point in that frame then remove it
			if (isPointCloseToPointInList(radius, p, existingPoints)) continue;
	...
	private void trackNeutrophilsFromStartingPointList(){
		...
		for (String imPath: jimages){
			...
			if (rollBall > 0){
				...
				JEXStatics.logManager.log("Rolling ball performed",1,this);
...
//			ImagePlus image = jimages.get(imageIndex).getImagePlus();
			...
			for (Trajectory t: tracks){
				...
				if (p != null) existingPoints.add(p);
			}
			...
			for (Point p: seeds){
				...
				tracks.add(t);
				JEXStatics.logManager.log("Added track to list.",1,this);
			...
			// Update status bar
			JEXStatics.logManager.log("Found cell in image " + index + " of " + len + ".",1,this);
...
//		imp = imp.resize((int)(image.getWidth()/binning));
	...
	private void analyze(){
		...
		for (int index=0; index<startinPositionX.size(); index++) rowNames[index] = "Cell "+index;
		...
		for (int i=0, len=startinPositionX.size(); i<len; i++)         col1[i] = ""+startinPositionX.get(i);
		for (int i=0, len=startinPositionY.size(); i<len; i++)         col2[i] = ""+startinPositionY.get(i);
		for (int i=0, len=wanglesV.size(); i<len; i++)                 col3[i] = ""+wanglesV.get(i);
		for (int i=0, len=totalDistancePerCell.size(); i<len; i++)     col4[i] = ""+totalDistancePerCell.get(i);
		for (int i=0, len=effectiveDistancePerCell.size(); i<len; i++) col5[i] = ""+effectiveDistancePerCell.get(i);
		for (int i=0, len=ciPerCell.size(); i<len; i++)                col6[i] = ""+ciPerCell.get(i);
		for (int i=0, len=wanglesA.size(); i<len; i++)                 col7[i] = ""+wanglesA.get(i);
...
//		for (int index=0; index<length; index++) rowNames[index] = "Cell "+index;
...
//		
////		output7 = DataFactory.makeValueTable(outputNames[6], columnArrayNames, rowNamesList, columnsList);
//		output7 = ValueWriter.makeValueTable(outputNames[6], columnArrayNames, columnsList);
