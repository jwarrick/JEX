 ...
 *
 */
public class JEX_DeathStar_PropagationLength extends ExperimentalDataCrunch{
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(FILE,"Measurements");
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if (rectangleData == null || !rectangleData.getTypeName().getType().equals(JEXData.ROI)) return false;
		...
		for(DimensionMap map : rectangleROI.keySet())
		{
			...
			rowOfInfo.clear();
			Iterator<PointList> itr = points.patternPointListIterator();
			while(itr.hasNext())
			{
				Point p = itr.next().get(0);
		...
		JEXData output1;
		output1 = FileWriter.makeFileObject(outputNames[0].getName(), writer.getPath(), "DeathStar");
