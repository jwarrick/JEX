 ...
 *
 */
public class JEX_MakeROIArray extends ExperimentalDataCrunch{
	
	public JEX_MakeROIArray(){}
	...
	public String getName() {
		String result = "Make ROI Pattern (auto)";
		return result;
	}
	...
	public String getInfo() {
		String result = "Make an array of rectangles (polygons to allow for angles), ellipses, lines (one segment), or points using the first 3 points of a Point ROI or Polyline ROI to define upper left (UL), UR, and LR (in that order).";
		return result;
	}
	...
	public TypeName[] getInputNames(){
		...
		inputNames[0] = new TypeName(ROI,"Array Definition ROI (3 Points)");
		return inputNames;
	}
...
//		Parameter p0 = new Parameter("Dummy Parameter","Lets user know that the function has been selected.",FormLine.DROPDOWN,new String[] {"true"},0);
		...
		Parameter p2 = new Parameter("Cols","Number of cols","2");
		Parameter p3 = new Parameter("Type","Type of roi",FormLine.DROPDOWN, new String[]{"Rectangle","Ellipse","Line","Point"},0);
		...
		Parameter p6 = new Parameter("ROI Origin","What part of the roi should be placed at the indicated points (i.e. does the array of points indicate where the upper-left corner should be placed?",FormLine.DROPDOWN,new String[]{"Center","Upper-Left","Upper-Right","Lower-Right","Lower-Left"},0);
		Parameter p7 = new Parameter("Staggered Pattern?","Should pattern be staggered (close-packed)?",FormLine.DROPDOWN,new String[] {"True","False"},1);
		Parameter p8 = new Parameter("First Row Is Staggered?", "Is the first row inset from the left in the staggered patter (ignored if not a staggered pattern)",FormLine.DROPDOWN,new String[]{"True","False"},1);
...
//		parameterArray.addParameter(p0);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		// Collect the inputs
		JEXData roiData = inputs.get("Array Definition ROI (3 Points)");
		...
		if(rows < 2 && cols < 2)
		{
			JEXStatics.logManager.log("Can't make an array with less than two locations!", 9, this);
			JEXStatics.statusBar.setStatusText("Can't make an array with less than two locations!");
			return false;
		}
		...
		boolean staggered = Boolean.parseBoolean(parameters.getValueOfParameter("Staggered Pattern?"));
		boolean staggeredFirst = Boolean.parseBoolean(parameters.getValueOfParameter("First Row Is Staggered?"));
		...
		// Get the points of the roi to copy to each location
		ROIPlus roip = getRoi(roiOrigin, roiType, roiWidth, roiHeight);
		if(roip == null || roip.getPointList().size() == 0)
		{
			JEXStatics.logManager.log("Couldn't get the points to make the roi!", 9, this);
		...
		for(DimensionMap roiM : roiMap.keySet())
		{
			...
			// Get the corners
			ROIPlus cornersRoi = roiMap.get(roiM);
			if(cornersRoi == null)
			{
				JEXStatics.logManager.log("Couldn't read the roi that defines the array!", 9, this);
				JEXStatics.statusBar.setStatusText("Couldn't read the roi that defines the array!");
				return false;
			}
			PointList corners = cornersRoi.getPointList();
			
			// Get the locations in the array
//			Map<DimensionMap, Point> pointMap = getArrayOfPoints(rows, cols, staggered, staggeredFirst, corners, rowName, colName);
//			if(pointMap == null)
//			{
//				JEXStatics.logManager.log("Couldn't make the array of points!", 9, this);
...
//			}
	...
	private PointList getPattern(int rows, int cols, boolean staggered, boolean staggeredFirst, PointList corners)
	{
		if(corners.size() < 3 || rows < 1 || cols < 1) return null;

		Point p1 = corners.get(0), p2 = corners.get(1), p3 = corners.get(2);
		double radius = Math.sqrt((p2.x-p1.x)*(p2.x-p1.x) + (p2.y-p1.y)*(p2.y-p1.y));
		double theta = Math.atan2((p2.y-p1.y),(p2.x-p1.x));
		Point origin = p1.getLocation();
		double theta2 = Math.atan2((p3.y-p1.y),(p3.x-p1.x)) - theta;
		double radius2 = Math.sqrt((p3.x-p1.x)*(p3.x-p1.x) + (p3.y-p1.y)*(p3.y-p1.y));
		double arrayHeight = radius2*Math.sin(theta2);

		double rSpace = ((double) arrayHeight)/((double) (rows-1));
		double cSpace = ((double) radius)/((double) (cols-1));
		boolean evenRow = false;
		boolean evenCol = false;
		...
		for(int r = 1; r <= rows; r++)
		{
			for(int c = 1; c <= cols; c++)
			{
				evenRow = (r%2 == 0);
				evenCol = (c%2 == 0);
				double dx = (Math.round((c-1)*cSpace));
				double dy = (Math.round((r-1)*rSpace));
				double RADIUS = Math.sqrt(dx*dx + dy*dy);
				double THETA = theta + Math.atan2(dy,dx);
				int x = origin.x + (int) Math.round(RADIUS*Math.cos(THETA));
				int y = origin.y + (int) Math.round(RADIUS*Math.sin(THETA));
	...
	private Map<DimensionMap, Point> getArrayOfPoints(int rows, int cols, boolean staggered, boolean staggeredFirst, PointList corners, String rowName, String colName)
	{
		if(corners.size() < 3 || rows < 1 || cols < 1) return null;
		...
		Map<DimensionMap, Point> pointMap = new HashMap<DimensionMap, Point>();
		for(int r = 1; r <= rows; r++)
		{
			for(int c = 1; c <= cols; c++)
			{
