 ...
 *
 */
public class JEX_FindEdges extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		Parameter p1 = new Parameter("Invert image before?","Inverts the LUT before proceeding to the edge detection",FormLine.DROPDOWN,new String[] {"false","true"},0);
		Parameter p2 = new Parameter("Invert image after?","Inverts the LUT after proceeding to the edge detection",FormLine.DROPDOWN,new String[] {"false","true"},0);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		for (DimensionMap dim:images.keySet()){
			...
			// invert if needed
			if (invBefore) bimp.invertLut();
			
			// Do the edge detection
			bimp.findEdges();
			
			// Do the invert if needed
			if (invAfter) bimp.invertLut();
