 ...
 *
 */
public class JEX_StackCalculation extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		Parameter p1 = new Parameter("Type","Type of calculation to perform",FormLine.DROPDOWN,new String[] {"Average","Add","Subtract"},0);
...
class StackCalculationHelper{
	...
	StackCalculationHelper(JEXEntry entry, JEXData imset, JEXData imset2, TypeName[] outputNames, ParameterSet parameters){
		...
		if (calculation == null || calculation.equals("")) return ;
		else if (calculation.equals("Average")) average() ;
		else if (calculation.equals("Add")) add() ;
		else if (calculation.equals("Subtract")) subtract() ;
	}
	...
	private void average(){
		...
		for (String impath: jimages){
			...
			// update status bar
			JEXStatics.logManager.log("Added image " + i + " of " + len + ".",1,this);
...
//		String newFileName = FunctionUtility.getNextName(localDir, "AverageStack", "Av");
...
//		FunctionUtility.imSave(averageImage, path);
		...
		JEXStatics.logManager.log("Saved outputted image.",1,this);
	...
	private void add(){
		...
		if (imset != null) map1 = ImageReader.readObjectToImagePathTable(imset);

		// get the tree map of the second imageset
		TreeMap<DimensionMap,String> map2 = new TreeMap<DimensionMap,String>();
		if (imset2 != null) map2 = ImageReader.readObjectToImagePathTable(imset2);
		...
		for (DimensionMap dim: map1.keySet()){
			...
			if (im1 == null || im1.getProcessor() == null) continue;
			...
			if (im2 == null || im2.getProcessor() == null) continue;
...
//			String localDir    = JEXWriter.getEntryFolder(entry);
//			String baseName    = file1.getName().substring(0,file1.getName().length()-4);
//			String newFileName = FunctionUtility.getNextName(localDir, baseName, "Av");
//			String path        = localDir + File.separator + newFileName;
