 ...
 *
 */
public class JEX_PercentageMaker extends ExperimentalDataCrunch{
	
	public JEX_PercentageMaker(){}
	...
	public String getName() {
		String result = "Percentage Maker";
		return result;
	}
	...
	public String getInfo() {
		String result = "Function that takes two points ROIs and calculates the percentage of one against the other.";
		return result;
	}
	...
	public TypeName[] getInputNames(){
		...
		inputNames[0] = new TypeName(ROI,"Total Point ROI");
		inputNames[1] = new TypeName(ROI,"Subset Point ROI");
		return inputNames;
	}
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(VALUE,"Percentage");
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if (subpointData == null || !subpointData.getTypeName().getType().equals(JEXData.ROI)) return false;
		JEXData totpointData = inputs.get("Total Point ROI");
		if (totpointData == null || !totpointData.getTypeName().getType().equals(JEXData.ROI)) return false;
		
		// Run the function
		TreeMap<DimensionMap,ROIPlus> subpointROI = RoiReader.readObjectToRoiMap(subpointData);
		TreeMap<DimensionMap,ROIPlus> totpointROI = RoiReader.readObjectToRoiMap(totpointData);

		ROIPlus pointsub;
		ROIPlus pointtot;
		TreeMap<DimensionMap,String> outputPercent = new TreeMap<DimensionMap,String>();
		int countsub = 0;
		int counttot = 0;
		int percent = 0;
		for(DimensionMap map : totpointROI.keySet())
		{
			pointsub = subpointROI.get(map);
			countsub = pointsub.getPointList().size();
			pointtot = totpointROI.get(map);
			counttot = pointtot.getPointList().size();
			percent  = countsub*100/counttot;
			outputPercent.put(map, ""+percent);
		}
		
		JEXData output1;
		output1 = ValueWriter.makeValueTable(outputNames[0].getName(), outputPercent);
