...
public class JEX_DistributeValues extends ExperimentalDataCrunch{
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p3 = new Parameter("Order","Fill row then go to next column (Row) or fill column and then go to next row (Column)",FormLine.DROPDOWN,new String[]{"Row","Column"});
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if(valueList == null || valueList.size() == 0) return false;
		...
		if(valueMap == null) valueMap = getValueMap(valueList, rows, cols, rowOrder);
