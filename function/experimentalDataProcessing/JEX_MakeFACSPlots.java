...
//		Parameter p1 = new Parameter("Octave Binary File","Location of the Octave binary file","/Applications/Octave.app/Contents/Resources/bin/octave");
		...
		Parameter p14 = new Parameter("Number of Frames Per Plot","Number of frames per plot","4");
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		result = R.source("makeJEXFACSPlot3.R");
		...
		for(int i = 0; i < maps.size()-(window-1); i++)
		{
			...
			String command = this.getRCommand_makeFileSublist(fileMap, maps, i, window);
			result = R.eval(command);
			command = this.getRCommand_makePlot(timeStamp);
			result = R.eval(command);
	...
	private String getRCommand_makeFileSublist(TreeMap<DimensionMap,String> fileMap, List<DimensionMap> maps, int start, int window)
	{
...
//		String measurementName = parameters.getValueOfParameter("Measurement Name");
//		double measurementIndex = Double.parseDouble(parameters.getValueOfParameter("Measurement Value"));
