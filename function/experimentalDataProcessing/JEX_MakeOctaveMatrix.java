 ...
 *
 */
public class JEX_MakeOctaveMatrix extends ExperimentalDataCrunch{
...
//		}
		...
		{
			text = new LSVList();
			text.add("% [Experiment = " + entry.getEntryExperiment() + "]:[Array = " + entry.getEntryTrayName() + "]:[Value = " + valueData.getTypeName().getName() + "]");
			text.add(""); // blank line to look prettier
			String indexOrder = "% " + var1 + "(ArrayRow,ArrayCol).data(";
			for(int i = 0; i < valueTable.size(); i++)
			{
				if(i != 0) indexOrder = indexOrder + ",";
	...
	public void finalizeTicket(Ticket t)
	{
		...
		for(JEXEntry entry : outputList.keySet())
		{
			...
			JEXData data = FileWriter.makeFileObject(outputNames[0].getName(), path, "Matrix");
