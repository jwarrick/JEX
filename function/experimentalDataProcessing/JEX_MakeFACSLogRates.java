 ...
 *
 */
public class JEX_MakeFACSLogRates extends ExperimentalDataCrunch{
	
	public JEX_MakeFACSLogRates(){}
	...
	public String getName() {
		String result = "Make FACS Log Rates";
		return result;
	}
	...
	public String getInfo() {
		String result = "Make an ARFF file of the log rates of XY information.";
		return result;
	}
...
//		Parameter p1 = new Parameter("Octave Binary File","Location of the Octave binary file","/Applications/Octave.app/Contents/Resources/bin/octave");
		...
		Parameter p7 = new Parameter("Number of Frames","Size of the rolling window in number of frames","4");
...
//		parameterArray.addParameter(p14);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		Integer frames = Integer.parseInt(parameters.getValueOfParameter("Number of Frames"));
		Double minInt = Double.parseDouble(parameters.getValueOfParameter("Minutes Between Frames"));
		...
		DataWindows tracks = new DataWindows(frames);
		Dim trackDim = null;
		int count = 0, percentage = 0;
		JEXStatics.statusBar.setProgressPercentage(0);
		double tInt = (frames-1)*(minInt/60);
		for(DimensionMap fileMap : fileTable.getIterator())
		{
			...
			List<DimensionMap> xMaps = timePoint.dimTable.getDimensionMaps(new DimensionMap("Measurement=x"));
			List<DimensionMap> yMaps = timePoint.dimTable.getDimensionMaps(new DimensionMap("Measurement=y"));
			tracks.increment();
			...
			for(int t = 0; t < xMaps.size(); t++)
			{
				...
				{
					//tracks.addPoint(xTrack, Math.log(x), Math.log(y));
			...
			Pair<DimensionMap,String> resultsTable = writeLogRatesTimePoint(tracks, trackDim, timeDim, "##0.00", tInt);
	...
	public Pair<DimensionMap,String> writeLogRatesTimePoint(DataWindows tracks, Dim trackDim, Dim timeDim, String timeFormat, Double tInt)
	{
		// "##0.00" time format
		...
		for(String trackNumString : trackDim.dimValues)
		{
			...
			if(track != null && track.isFilled())
			{
				...
				dataPointMap.put("Measurement", "dx");
				avgData.put(dataPointMap.copy(), track.dx()/tInt);
				dataPointMap.put("Measurement", "dy");
				avgData.put(dataPointMap.copy(), track.dy()/tInt);
				dataPointMap.put("Measurement", "Persistence");
				avgData.put(dataPointMap.copy(), track.r()/track.l());
			}
			count = count + 1;
		...
		Dim measurementDim = new Dim("Measurement",new String[]{"dx","dy","Persistence"});
		...
		String tablePath = JEXTableWriter2.writeTable("LogRateData", table);
