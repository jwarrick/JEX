...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		for(DimensionMap map : maps)
		{
			...
			if(stackToProject != null) stackToProject.flush();
			...
			// Results are... Object[] = {average intensity in roi for this stack, thresholded projected Image}
		...
		JEXData output3 = FileWriter.makeFileObject(outputNames[2].getName(), filePath, "CSV");
	...
	/**
	 * Takes each frame of each stack applies the threshold and gets the intensity
	 * Returns result in a 2 object Object[] = {average intensity in roi for each stack,
	 * thresholded images}
	 * @param imageData
	 * @param stackMaps
	 * @param thresh
	...
	private static Object[] getAverage(JEXEntry entry, JEXData imageData, List<DimensionMap> stackMaps, double thresh, JEXData regionToAnalyze, int statusIndex, int statusTotal)
	{
		...
		for(DimensionMap map : stackMaps)
		{
			...
			{
				JEXStatics.logManager.log("Couldn't find image for: " + map, 0, "JEX_OscillatoryAdhesionAnalysis");
			}
			count = count + 1;
	...
	private static String saveImage(FloatProcessor imp, int bitDepth)
	{
		...
		JEXStatics.logManager.log("Writing image to: " + finalPath, 1, "JEX_OscillatoryAdhesionAnalysis");
		return finalPath;
	}
