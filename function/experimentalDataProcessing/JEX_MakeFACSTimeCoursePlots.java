 ...
 *
 */
public class JEX_MakeFACSTimeCoursePlots extends ExperimentalDataCrunch{
...
//		Parameter p1 = new Parameter("Octave Binary File","Location of the Octave binary file","/Applications/Octave.app/Contents/Resources/bin/octave");
		...
		Parameter p6 = new Parameter("Variable Shading?","Whether to allow the shade of the lines to vary to enhance visibility of individual tracks",FormLine.DROPDOWN,new String[]{"True","False"},0);
		Parameter p6b = new Parameter("Normalized?","Whether to normalize all the values to be between 0 and 1 (min and max)",FormLine.DROPDOWN,new String[]{"True","False"},1);
...
//		parameterArray.addParameter(p14);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		Table<Double> firstTrack = JEXTableReader2.getNumericTable(fileTreeMap.firstEntry().getValue());
		...
		for(DimensionMap fileMap : fileTable.getIterator(new DimensionMap(), skipN))
		{
			Table<Double> track = JEXTableReader2.getNumericTable(fileTreeMap.get(fileMap));
		...
		for(String color : colorDim.dimValues)
		{
			...
			for(DimensionMap fileMap : fileTable.getIterator(new DimensionMap(), skipN))
			{
				Table<Double> track = JEXTableReader2.getNumericTable(fileTreeMap.get(fileMap), new DimensionMap("Measurement=1," + colorDimName + "=" + color));
