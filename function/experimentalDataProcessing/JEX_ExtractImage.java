 ...
 *
 */
public class JEX_ExtractImage extends ExperimentalDataCrunch {
	...
	public ParameterSet requiredParameters() {
		Parameter p1 = new Parameter("Only once","Extract the image only once and distribute it to all the other entries",FormLine.DROPDOWN,new String[] {"true","false"},0);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		// Run the function
		ImagePlus im = null;
		if (auto && JEX_ExtractImage.cellImage!=null){
...
//		String newFileName = FunctionUtility.getNextName(localDir, "Croped.TIF", "Ex");
//		String finalPath   = localDir + File.separator + newFileName;
//		JEXStatics.logManager.log("Saving in "+finalPath,1,this);
//		FunctionUtility.imSave(imp, "false", 16, finalPath);
		
		// Save the image
		JEXStatics.logManager.log("Saving image",1,this);
...
class VisualExtractionHelperFunction implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	VisualExtractionHelperFunction(JEXEntry entry, List<String> imset, TypeName[] outputNames, ParameterSet params){
		...
		JEXStatics.logManager.log("Placing in imagepanel "+images.get(0),1,this);
	...
	public void finishIT() {
		...
		JEX_ExtractImage.cellImage = imp.createImage();
	}
