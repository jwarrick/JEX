	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		try
		{
	       // Do file stuff 
			...
			if (pointData1 == null || !pointData1.getTypeName().getType().equals(JEXData.ROI)) return false;
			JEXData image = inputs.get("Images");
			if (image == null || !image.getTypeName().getType().equals(JEXData.IMAGE)) return false;
			...
			// This creates a line at the top of the CSV list showing a numbered list of the points
			...
			// This creates a line underneath that CSV list showing the x coordinates of each point
			for(DimensionMap map : pointROI1.keySet())
			{
			...
			// This creates a line underneath that CSV list showing the y coordinates of each point
			for(DimensionMap map : pointROI1.keySet())
			{
				pointy = pointROI1.get(map);
				if (pointy == null) continue;
			...
			for(DimensionMap map : pointROI1.keySet())
			{
				point = pointROI1.get(map); // This gets the points corresponding to the JEX entry
		    ...
		    JEXData output1 = FileWriter.makeFileObject(outputNames[0].getName(), finalPath,"Func");
			// Set the outputs
