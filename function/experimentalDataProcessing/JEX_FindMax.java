 ...
 *
 */
public class JEX_FindMax extends ExperimentalDataCrunch {
	...
	public ParameterSet requiredParameters() {
		Parameter p1 = new Parameter("Automatic","Enable visual interface",FormLine.DROPDOWN,new String[] {"true","false"},1);
		Parameter p2 = new Parameter("Cell Radius","Cell radius in pixels (e.g. 3 to 30)","15");
		Parameter p3 = new Parameter("Mode","Which type of calculation to perform?",FormLine.DROPDOWN,new String[] {"Threshold","Fixed number"},1);
		Parameter p4 = new Parameter("Threshold","Cell radius in pixels (e.g. 3 to 30)","15");
		Parameter p5 = new Parameter("Number of Cells","Cell radius in pixels (e.g. 3 to 30)","15");
		Parameter p6 = new Parameter("Loop order","Reverse looping order?",FormLine.DROPDOWN,new String[] {"Normal","Reverse"},1);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		// Run the function
		FindMaxHelperFunction graphFunc = new FindMaxHelperFunction(entry,data1,data2,outputNames,parameters);
		graphFunc.doit();
...
class FindMaxHelperFunction implements GraphicalCrunchingEnabling, ImagePanelInteractor{
	...
	FindMaxHelperFunction(JEXEntry entry, JEXData imset, JEXData jroi, TypeName[] outputNames, ParameterSet parameters){
		// Pass the variables
		this.imset  = imset;
		this.entry  = entry;
		...
		if (jroi != null){
			...
			this.roi = (roip==null) ? null : roip.roi;
		}
		...
		////// Get params
		//boolean auto  = Boolean.parseBoolean(parameters.getValueOfParameter("Automatic"));
		...
		if (!this.ord) index    = this.images.size()-1;
		...
		wrap.addStep(0, "Select roi", new String[] {"Automatic","Cell Radius","Threshold","Number of Cells","Mode"});
	...
	private void displayImage(int index){
		...
		JEXStatics.logManager.log("Placing in imagepanel "+imPath,1,this);
	...
	public void runStep(int index) {
		...
		if (pLists == null) pLists = new TreeMap<DimensionMap,PointList>();
	...
	public void finishIT() {
		...
		for (int index=0; index<dimensions.size(); index++){
			...
			if (pList == null || pList.size() == 0){
				JEXStatics.logManager.log("Runing missed step "+map,1,this);
				runStep(index);
			}
...
//			index ++;
		...
		for (int index=0; index<dimensions.size(); index++){
			...
			JEXStatics.logManager.log("Finished processing " + atStep + " of " + images.size() + ".",1,this);
		}
