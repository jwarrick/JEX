 ...
 *
 */
public class JEX_RotateImageUsingLine extends ExperimentalDataCrunch{
	
	public JEX_RotateImageUsingLine(){}
	...
	public String getName() {
		String result = "Rotate Image using Line ROI";
		return result;
	}
	...
	public String getInfo() {
		String result = "Function that allows you to rotate a set of images based on a Line ROI within each image.";
		return result;
	}
	...
	public TypeName[] getInputNames(){
		...
		inputNames[1] = new TypeName(ROI,"Line ROI");
		return inputNames;
	}
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(IMAGE,"Rotated Image");
	...
	public ParameterSet requiredParameters() {
		Parameter p0 = new Parameter("Image Orientation","Rotate to correct an image to either vertical or horizontal",FormLine.DROPDOWN,new String[] {"Vertical","Horizontal"},1);
		Parameter p1 = new Parameter("Interpolation Method","Reassigns Pixel Intensities Based on This Method",FormLine.DROPDOWN,new String[] {"Bilinear","Bicubic","Nearest Neighbor"},0);
...
//		parameterArray.addParameter(p6);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if (imageFiles == null || !imageFiles.getTypeName().getType().equals(JEXData.IMAGE)) return false;

		// Collect the inputs
		JEXData lineData = inputs.get("Line ROI");
		if (lineData == null || !lineData.getTypeName().getType().equals(JEXData.ROI)) return false;
		
		// Run the function
		TreeMap<DimensionMap,String> imageList = ImageReader.readObjectToImagePathTable(imageFiles);
		TreeMap<DimensionMap,ROIPlus> lineROI = RoiReader.readObjectToRoiMap(lineData);
		TreeMap<DimensionMap,String> outputMap = new TreeMap<DimensionMap,String>();
		String interp = parameters.getValueOfParameter("Interpolation Method");
		int interpMode = ImageProcessor.NEAREST_NEIGHBOR;
		if (interp.equals("Bilenar")) interpMode = ImageProcessor.BILINEAR;
		else if (interp.equals("Bicubic")) interpMode = ImageProcessor.BICUBIC;
		...
		String orient = parameters.getValueOfParameter("Image Orientation");
		double orientangle = 0;
		if (orient.equals("Vertical")) orientangle = 90;
		...
		for(DimensionMap map : imageList.keySet())
		{
			...
			if (line == null || line.roi == null) continue;
			point1 = line.getPointList().get(0);
			point2 = line.getPointList().get(1);
			if (orientangle == 0 || point1.x > point2.x) 
			{
				pointcheck = (Point)point2.clone(); 
				point2 = point1;
				point1 = pointcheck;
			}
//			if (orientangle == 90 || point1.y > point2.y)
//			{
//				pointcheck = (Point)point2.clone();
//				point2 = point1;
//				point1 = pointcheck;
...
//	
//	AdjustImageHelperFunction(JEXData imset, JEXEntry entry, String[] outputNames, ParameterSet parameters){
//		// Pass the variables
//		this.imset  = imset;
