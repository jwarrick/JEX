 ...
 *
 */
public class JEX_FileStackToImageStack extends ExperimentalDataCrunch{
	
	public JEX_FileStackToImageStack(){}
	...
	public String getName() {
		String result = "File to Image stack";
		return result;
	}
	...
	public String getInfo() {
		String result = "Extract a stk file into a JEX image stack.";
		return result;
	}
	...
	public TypeName[] getInputNames(){
		...
		inputNames[0] = new TypeName(FILE,"File stack");
		return inputNames;
	}
	...
	public TypeName[] getOutputs(){
		...
		defaultOutputNames[0] = new TypeName(IMAGE,"Image stack");
	...
	public ParameterSet requiredParameters() {
		Parameter p0 = new Parameter("Dimension name","Name of the dimension applied to the stack","0.0");
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs){
		...
		if (!data.getTypeName().getType().equals(JEXData.FILE)) return false;
		
		// Get the file name
		String fileName = FileReader.readFileObject(data);
		
		// Open an imageJ stack
		ImagePlus imstack = new ImagePlus(fileName);
		
		// Save each image of the stack into a new file
		ImageStack stack = imstack.getImageStack();
		...
		for (int i=1; i<=stackSize; i++)
		{
			...
			ImagePlus      im  = new ImagePlus("", imp);
			
			// Save the image
			String imPath    = JEXWriter.saveImage(im);
		...
		String[] indeces = indexList.toArray(new String[0]);
		String   dimName = this.parameters.getValueOfParameter("Dimension name");
		
		// Make the stack output
		JEXData output = ImageWriter.makeImageStack(outputNames[0].getName(), files, indeces, dimName);
