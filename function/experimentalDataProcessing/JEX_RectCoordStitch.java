 ...
 *
 */
public class JEX_RectCoordStitch extends ExperimentalDataCrunch{
	
	public static final int LtoR = 0, TtoB = 1;
	
	public JEX_RectCoordStitch(){}
	...
	public ParameterSet requiredParameters() {
		...
		Parameter p6 = new Parameter("Image Order","Order in which the images were taken",FormLine.DROPDOWN,new String[] {"Row First LtoR","Column First TtoB"},0);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		for(DimensionMap partialMap : partialTable.getDimensionMaps())
		{
			...
			File stitchedFile = JEX_CoordStitch.stitch(entry, imageData, mapsToGet, imageCoords, scale, normalize, multiplier, bitDepth);
	...
	private PointList getMovements(int horDxImage, int horDyImage, int verDxImage, int verDyImage, int rows, int cols, int order, double scale)
	{
		...
		else // Go through row first (i.e. index col first)
		{
			for(int r = 0; r < rows; r++)
			{
				for(int c = 0; c < cols; c++)
