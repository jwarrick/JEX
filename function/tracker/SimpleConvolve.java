...
public class SimpleConvolve {
	...
	public ImagePlus start(ImagePlus image, ImagePlus conv, Rectangle roi){
		...
		// Do the work
		result = analyzeImage();
		JEXStatics.logManager.log("Convolution finished... result saved ",1,this);
//		result.show();
	...
	public ImagePlus analyzeImage(){// launch the calculation
...
//		result = convolutionMatrix(imageMatrix, imageSmall);
		...
		stopwatch.stop();
		JEXStatics.logManager.log("First convolution took "+stopwatch.toString(),1,this);
	...
	public ImageProcessor convolve(ImageProcessor arg, ImageProcessor kernel){
		JEXStatics.logManager.log("Averaging typical cell image ",1,this);
...
//		float maximum = 0;
		...
		JEXStatics.logManager.log("Convolving ... ",1,this);
...
//		result = getAdjustedImage(result);
	...
	public ImagePlus convolutionMatrix(float[][] fimage, float[][] simage){
		...
		//			JEXStatics.logManager.log("  ** Tracker: image size "+w+", "+h);
		...
		for (int i=startY, len=endY; i<len; i++){
			...
			for (int j=startX, len2=endX; j<len2; j++){
				...
				//JEXStatics.logManager.log("convolving point x="+i+", y="+j);
			}
		}
	...
	public float scoreConvolution(int x, int y, float[][] fimage, float[][] simage, float average2){
		//			JEXStatics.logManager.log("  ** Tracker: Convolving position "+x+", "+y);
