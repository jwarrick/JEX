...
public class TrackStatistics {
	...
	public TrackStatistics(XTrajectorySet tracks){
		...
		JEXStatics.logManager.log("Number of tracks found "+numberTracks,1,this);
	}
	...
	public TrackStatistics(Trajectory[] trajectories){
		...
		for (Trajectory t: trajectories) this.trajectories.add(t);
		numberTracks = (trajectories==null)? 0 : this.trajectories.size();
		JEXStatics.logManager.log("TrackAnalyzer ---> Number of tracks found "+numberTracks,1,this);
	}
	...
	public void startAnalysis(){
		...
		if (mode == SINGLEDISPLACEMENT){			
			...
			while (listNotEmpty){
				// if the requested number of vectors has been reached, stop there
				...
				for (int i=0; i<numberTracks; i++){
					...
					// if there is not enough displacements on this list, skip it
					if (vs.size() <= index) continue;
		...
		if (mode == SINGLEDISPLACEMENT){
			...
			int plength = 0;
			for (double p: persistenceTimes){
				if (p == Double.POSITIVE_INFINITY) continue;
	...
	public Trajectory loadTrajectory(int index,int min, int max){
		if ((index<0)||(index>=trajectories.size())) {return null;}
