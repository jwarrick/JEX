...
//		JEXStatics.logManager.log("Loading trajectory statistics", 2, this);
	}
	...
	public void startAnalysis(int min, int max){
		...
		while (nextFrame <= max){
			...
			// calculate angles if the norm of the vector is 0 then the angle is irrelevant
			if (w > 0) {
		...
		meanVector   = (vectors.size() == 0) ? new Vect(0,0) : vectors.mean();        // The mean Vector
		meanVelocity = (velocities.size() == 0) ? 0 : VectorUtility.mean(velocities); // The mean velocity
		meanAngle    = (vectors.size() == 0) ? 0 : vectors.meanAngle();               // The mean angle
		CI           = (distCI==0) ? 0 : effectiveDistance / distCI;                  // the chemotactic angle
		SCI          = 0;                                                             // The segmented chemotactic index
	}
