...
//	private Color backgroundColor = DisplayStatics.background;
	...
	protected void initiate(){
		...
		if (!right) this.add(Box.createRigidArea(new Dimension(sideSpace,sideSpace)),BorderLayout.LINE_END);
		if (!left) this.add(Box.createRigidArea(new Dimension(sideSpace,sideSpace)),BorderLayout.LINE_START);
		if (!top) this.add(Box.createRigidArea(new Dimension(sideSpace,sideSpace)),BorderLayout.PAGE_START);
		if (!bottom) this.add(Box.createRigidArea(new Dimension(sideSpace,sideSpace)),BorderLayout.PAGE_END);
	}
	...
	public void refresh(){
		...
		if (menuPanel!=null) thisContentPane.add(menuPanel,BorderLayout.PAGE_START);
		if (centerPane!=null) thisContentPane.add(centerPane,BorderLayout.CENTER);
		if (statusbar!=null) thisContentPane.add(statusbar,BorderLayout.PAGE_END);
		if (leftPane!=null) thisContentPane.add(leftPane,BorderLayout.LINE_START);
		this.repaint();
	}
