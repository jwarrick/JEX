...
public class JRoundedCollapsablePanel extends JPanel implements ActionListener{
	...
	public void initialize(){
		...
		if (!right) this.add(Box.createRigidArea(new Dimension(3,3)),BorderLayout.LINE_END);
		if (!left) this.add(Box.createRigidArea(new Dimension(3,3)),BorderLayout.LINE_START);
		if (!top) this.add(Box.createRigidArea(new Dimension(3,3)),BorderLayout.PAGE_START);
		if (!bottom) this.add(Box.createRigidArea(new Dimension(3,3)),BorderLayout.PAGE_END);
	}
...
//	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == reduce){
			JEXStatics.logManager.log("Reducing panel", 1, this);
			reduce();
		}
		if (e.getSource() == expand){
			JEXStatics.logManager.log("Expanding panel", 1, this);
			expand();
		}
	}
}
