...
public class JEXEntry implements Comparable<JEXEntry>{
	...
	JEXDB   parent;
	TreeMap<String, TreeMap<String, JEXData>> dataList;
...
//	public JEXEntry(String id, String expName, String arrayname, int row, int col,
//			String mdate, String date, String author, String entryinfo){
//		
//		info = new TreeMap<String,String>();
//		info.put(EXPERIMENT, expName);
//		info.put(TRAY, arrayname);
//		info.put(DATE, date);
//		info.put(MODIFDATE, mdate);
//		info.put(INFO, entryinfo);
//		info.put(X, ""+row);
//		info.put(Y, ""+col);
//		info.put(EID, id);
//	}
	 ...
	 * @param trayName
	 */
	public void setEntryTrayName(String trayName){
		info.put(TRAY, trayName);
	}
	...
	public String getEntryTrayName(){
		String result = (info.get(TRAY)==null)? "No Tray" : info.get(TRAY);
		return result;
	}
	...
	public String getDate(){
		...
		if (date == null || date.equals("NA") || date.equals("")) return DateUtility.getDate();
	...
	public String getModificationDate(){
		...
		if (date == null || date.equals("NA") || date.equals("")) setModificationDate(DateUtility.getDate());
	...
	public String findModificationDate(){
		...
		TreeMap<String, TreeMap<String, JEXData>> dataList = getDataList();
		if (dataList == null || dataList.size() == 0) {
			if (this.getDate() != null) return this.getDate();
			else return DateUtility.getDate();
		}
	...
	/**
	 * Add a data element to this entry... If flag boolean is true an existing data of the same name and type 
	 * will be overwritten
	 * @param data
	 * @param overwrite
	 * @return success flag
	...
	public boolean addData(JEXData data, boolean overwrite)
	{
		...
		try
		{
			data.setParent(this); // Important because data doesn't have parent yet.

			// Get the object keys TYPE and NAME
			String type = data.getDataObjectType();
			...
			// If a data object already exists in that spot, overwrite if the flag allows, return if not
			if (existingData != null && !overwrite) return false;
			
			// If a data with the same type and name exists and you want to overwirte it, delete it before
		...
		{
			String value = "";
			if (data != null && data.getDataObjectName() != null) value = data.getDataObjectName(); 
			JEXStatics.logManager.log("Error adding data to the database "+value, 0, this);
...
//		boolean autoDel = PrefsUtility.getAutoRemoveOnDelete();
//		if (autoDel)
//		{
//			File detachedFile = new File(JEXWriter.getDatabaseFolder() + File.separator + data.getDetachedRelativePath());
//			try
//			{
//				FileUtils.deleteDirectory(detachedFile.getParentFile());
//				JEXStatics.jexManager.saveCurrentDatabase();
//			} 
//			catch (IOException e)
//			{
//				JEXStatics.logManager.log("Couldn't remove data folder and contents for " + detachedFile.getParent(), 0, this);
//				e.printStackTrace();
//				success = false;
//			}
//		}
	...
	/**
	 * Return the data contained in this entry of TypeName TYPENAME
	 * if non exists returns null
	 * @param typeName
	 ...
	 */
	public TreeMap<String, TreeMap<String, JEXData>> getDataList(){
		if (dataList == null) dataList = new TreeMap<String, TreeMap<String, JEXData>>();
	 ...
	 * @param dataList
	 */
	public void setDataList(TreeMap<String, TreeMap<String, JEXData>> dataList){
	...
	public String toString(){
		...
		result = result + " - " +  getEntryTrayName();
	...
	public String toSimpleString(){
		String result = getEntryExperiment() + " - " +  getEntryTrayName() + " - " +  getTrayX() + "." + getTrayY();
		return result;
	}
...
//		return this.getEntryID().compareTo(f.getEntryID());
	...
	public boolean equals(Object o){
		if (!(o instanceof JEXEntry)) return false;
