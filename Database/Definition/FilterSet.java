...
public class FilterSet extends Vector<Filter>{
	...
	public void addFilter(Filter filter){
		...
		for (Filter f: this){
			// If the filter typename already exists in the filterset, then only add
	...
	public void removeFilter(Filter filter){
		...
		for (Filter f: this){
			// If the filter typename already exists in the filterset, then only remove
			// the values ot the value set
			if (f.getType().equals(type) && f.getName().equals(name)){
				for (String value: filter.getValues()){
					f.removeValue(value);
					if (f.getValues().size() == 0) sweep.add(f);
				}
			}
			// Else do nothing
			else {}
...
//			if (f.getValues().size() == 0) this.remove(f);
	...
	public void print()
	{
		JEXStatics.logManager.log("-----------------", 1, this);
		JEXStatics.logManager.log("Filter Set output", 1, this);
		...
		for (Filter f: this)
		{
			...
			JEXStatics.logManager.log("   Filter 1: TYPE="+type+" NAME="+name+" VALUES="+values.toString(), 1, this);
		}
