...
public class JEXDBInfo {
	...
	/**
	 * sets the jex file path of this databaseInfo object
	 * causes reload using the information at this new location
	 * @param jexPath
	 */
	...
	public String getType()
	{
		...
		if (type == null || type.equals("")) type = JEXDB.LOCAL_DATABASE;
		return type;
	}
...
//			Experiment exp = new Experiment(expName,expInfo,expDate,expMDate,expAuthor,expNumber);
//			
//			// Add it to the table
...
//	}
		...
		for (XPreferences label: labels)
		{
			...
			for (XPreferences value: label.getChildNodes())
			{
				try 
				{
					...
					JEXStatics.logManager.log(labelValue + " has color of " + labelColor, 0, this);
					...
					JEXStatics.logManager.log(labelValue + " now has matching color " + (ColorPallet.colorToString(color).equals(value.get(JEXDBInfo.DB_LABELCOLORCODE_COLOR))) + " and colorIndex = " + ColorPallet.getColorIndex(color), 0, this);
					result.setColorForLabel(labelName, labelValue, color);
				}
				catch (Exception e)
				{
					JEXStatics.logManager.log("Error in reading label "+labelName, 1, null);
					continue;
				}
				count = count + 1;
			}
	...
	public static JEXDBInfo[] findDatabasesInRepository(Repository rep)
	{
		...
		if (!toLoad.exists()){return new JEXDBInfo[0];}
		if (!toLoad.isDirectory()){return new JEXDBInfo[0];}

		// Create the result list and loop through the folders
		File[] subFiles = toLoad.listFiles();
		if (subFiles == null) return new JEXDBInfo[0];
