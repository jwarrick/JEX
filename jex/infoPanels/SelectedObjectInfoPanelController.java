...
public class SelectedObjectInfoPanelController extends InfoPanelController
{
	...
	public void viewedEntryOrDataChange()
	{
		...
		// get the entry TNV
		TreeMap<String,TreeMap<String,JEXData>> entrymap = viewedEntry.getDataList();
		...
		objectValue = (objectmap == null) ? "" : objectmap.get(objName).getDictionaryValue();
	...
	class SelectedObjectInfoPanel extends InfoPanel implements ImageDisplayController, MouseListener, ActionListener{
		...
		private void rebuild(){
			...
			JLabel label4 = new JLabel(objectType);
			...
			else
			{
				...
				JComponent comp = makeDataComponent(selectedObject.getType(),selectedObject.getName(),objectValue);
				if (comp != null) 
				{
		...
		private JComponent makeDataComponent(String type, String name, String value)
		{
			if (type.equals(JEXData.IMAGE))
			{
				...
				if (image.getProcessor() == null) return null;
...
//					XMLPreferences_XElement xdata = ((JEXData)data).getViewingXMLElement();
