...
public class NotesAndFilesWindow extends JDialog {
	...
	public NotesAndFilesWindow(){
		SSCenter.defaultCenter().connect(JEXStatics.jexManager, JEXManager.DATASETS, this, "arrayChanged", (Class[])null);
	...
	private void initialize(){
		...
		Image icondb        = JEXStatics.iconRepository.getImageWithName(IconRepository.DATABASE, 20, 20);
		Image iconOverdb    = JEXStatics.iconRepository.getImageWithName(IconRepository.DATABASE, 20, 20);
		Image iconPresseddb = JEXStatics.iconRepository.getImageWithName(IconRepository.DATABASE, 20, 20);
		...
		dbLevel.setClickedColor(DisplayStatics.menuBackground, DisplayStatics.lightBackground);
		...
		SSCenter.defaultCenter().connect(dbLevel, SignalMenuButton.SIG_ButtonClicked_NULL, this, "viewDBlevel", (Class[])null);
		menuPanel.add(Box.createHorizontalStrut(2));
		menuPanel.add(dbLevel);
		
		Image iconexp        = JEXStatics.iconRepository.getImageWithName(IconRepository.EXPERIMENT_ICON, 20, 20);
		Image iconOverexp    = JEXStatics.iconRepository.getImageWithName(IconRepository.EXPERIMENT_ICON, 20, 20);
		Image iconPressedexp = JEXStatics.iconRepository.getImageWithName(IconRepository.EXPERIMENT_ICON, 20, 20);
		expLevel.setBackgroundColor(DisplayStatics.background);
		expLevel.setForegroundColor(DisplayStatics.lightBackground);
		expLevel.setClickedColor(DisplayStatics.menuBackground, DisplayStatics.lightBackground);
		...
		SSCenter.defaultCenter().connect(expLevel, SignalMenuButton.SIG_ButtonClicked_NULL, this, "viewEXPlevel", (Class[])null);
		menuPanel.add(Box.createHorizontalStrut(5));
		menuPanel.add(expLevel);
		
		Image icontray        = JEXStatics.iconRepository.getImageWithName(IconRepository.TRAY_ICON, 20, 20);
		Image iconOvertray    = JEXStatics.iconRepository.getImageWithName(IconRepository.TRAY_ICON, 20, 20);
		Image iconPressedtray = JEXStatics.iconRepository.getImageWithName(IconRepository.TRAY_ICON, 20, 20);
		...
		SSCenter.defaultCenter().connect(trayLevel, SignalMenuButton.SIG_ButtonClicked_NULL, this, "viewTraylevel", (Class[])null);
		menuPanel.add(Box.createHorizontalStrut(2));
	...
	public void arrayChanged(){
		if (JEXStatics.jexManager.getCurrentDatabase() == null){
			...
			centerPanelTMP.setLayout(new BoxLayout(centerPanelTMP,BoxLayout.PAGE_AXIS));
			...
			Iterator<JEXEntry> itr = JEXStatics.jexManager.getCurrentDatabase().getEntries().iterator();
			if (itr.hasNext()){
				JEXEntry entry = itr.next();
				String expFolder = JEXWriter.getAttachedFolderPath(entry,JEXEntry.EXPERIMENT, true, false);
				String arrayFolder = JEXWriter.getAttachedFolderPath(entry,JEXEntry.TRAY, true, false);
				expNotes.setFolderViewed(expFolder,JEXWriter.getAttachedNotesPath(entry,JEXEntry.EXPERIMENT, true, false));
				arrayNotes.setFolderViewed(arrayFolder,JEXWriter.getAttachedNotesPath(entry,JEXEntry.TRAY, true, false));
			}
			else 
			{
			...
			dbNotes.setFolderViewed(dbFolder,JEXWriter.getAttachedNotesPath(null, "", true, false));
