...
public abstract class JEXTabPanelController {
	...
	public void saveSplitPaneOptions(JSplitPane split)
	{
		...
		else // use the size of the left pane
		{
	...
	public void imposeSplitPaneOptions(JSplitPane split)
	{
		...
		if(this.resizeWeight == 1.0) // set the size of the divider location to produce the desired size of the RIGHT pane
		{
			split.setDividerLocation(leftWidth + rightWidth - this.fixedPanelWidth);
		}
		else // set the size of the divider location to produce the desired size of the LEFT pane
		{
	...
	{
		JEXStatics.logManager.log("Emitting close signal", 1, this);
