...
public class ExperimentalViewMode extends JPanel{
	...
	public void select2D(){
		JEXStatics.logManager.log("Switching to 2D viewing mode",1,this);
		String viewMode = JEXStatics.jexManager.getArrayViewingMode();
		if (viewMode.equals(JEXManager.ARRAY_2D)) return;
		JEXStatics.jexManager.setArrayViewingMode(JEXManager.ARRAY_2D);
	}

	public void select1D(){
		JEXStatics.logManager.log("Switching to 1D viewing mode",1,this);
		String viewMode = JEXStatics.jexManager.getArrayViewingMode();
		if (viewMode.equals(JEXManager.ARRAY_1D)) return;
		JEXStatics.jexManager.setArrayViewingMode(JEXManager.ARRAY_1D);
	}

	public void select0D(){
		JEXStatics.logManager.log("Switching to 0D viewing mode",1,this);
		String viewMode = JEXStatics.jexManager.getArrayViewingMode();
		if (viewMode.equals(JEXManager.ARRAY_0D)) return;
		JEXStatics.jexManager.setArrayViewingMode(JEXManager.ARRAY_0D);
	}
	
	public void selectLink(){
		JEXStatics.logManager.log("Switching to tree viewing mode",1,this);
		String viewMode = JEXStatics.jexManager.getArrayViewingMode();
		if (viewMode.equals(JEXManager.ARRAY_LINK)) return;
