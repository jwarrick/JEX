...
public class FilterPanel extends JPanel implements ActionListener, MouseListener{
	...
	public FilterPanel(String type, String labelName, TreeSet<String> labels, JEXStatisticsRightPanel parent){
		this.labels = labels;
	...
	public FilterPanel(String type, String labelName, String[] labels, JEXStatisticsRightPanel parent){
	...
	private void initialize(){
		...
		if (labelName.equals("Valid")) expanded = true;
	}
...
//		setIsFiltered(filt);
	...
	public boolean isAllValuesFiltered(){
		for (FilterValuePanel labelPane: panelList.values()){
			if (!labelPane.isFiltered()) return false;
		}
		return true;
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == groupButton){
			if (this.isGroupedBy) parent.removedGroup(labelType, labelName);
			else parent.addedGroup(labelType, labelName);
		}
		if (e.getSource() == viewButton){
			boolean allFiltered = isAllFiltered;
			for (String labelValue: labels){
				if (allFiltered) parent.removedFilter(labelType, labelName, labelValue);
				else parent.addedFilter(labelType, labelName, labelValue);
			}
		}
	...
	class FilterValuePanel extends JPanel implements ActionListener{
		...
		FilterValuePanel(String type, String labelName, String labelValue){
			this.labelType  = type;
			this.labelName  = labelName;
		...
		public void setFiltered(boolean isFiltered){
			...
			if (isFiltered) selectButton.setPressed(true);
			else selectButton.setPressed(false);
		}
		
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == selectButton){
				if (isFiltered) parent.removedFilter(labelType, labelName, labelValue);
				else parent.addedFilter(labelType, labelName, labelValue);
			}
		}
