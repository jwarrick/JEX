...
//		files = new HashMap<Point,TreeMap<DimensionMap,String>>();
	...
	public void setTray(Tray currentTray)
	{
		this.currentTray = currentTray;
		if (cellControllers == null) rebuild();
		refreshGUI();
	}
...
//		this.files = files;
	...
	{
		// Get the tray
		if (this.currentTray == null) return;
		...
		// Set the selection
		if (selected) JEXStatics.jexManager.addEntryToSelection(entry);
		else JEXStatics.jexManager.removeEntryFromSelection(entry);
	}
	...
	public void refreshGUI()
	{
		...
		if (selectionArray == null) selectionArray = new HashMap<Point,Boolean>();
		...
		if (this.parentController.files == null) this.parentController.files = new HashMap<Point,Vector<Pair<DimensionMap,String>>>();
		for (int i=0; i<rows; i++){
			for (int j=0; j<cols; j++){
				// set the selection status
				Boolean selected = selectionArray.get(new Point(i,j));
				if (selected!= null && selected) this.cellControllers[i][j].setSelected(true);
				else  this.cellControllers[i][j].setSelected(false);
	...
	{
		if (currentTray == null) selectionArray = new HashMap<Point,Boolean>();
		else
		{
