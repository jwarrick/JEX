...
public class PrefsUtility {
	...
	public static final String ATT_RSERVEBINARY = "R Serve";
	public static final String ATT_RSERVESTARTDELAY = "R Serve Start Delay";
	...
	public static boolean getConsolidateDatabase()
	{
		...
		catch(Exception e) // Repair the preference
		{
			...
			savePrefs();
			JEXStatics.logManager.log("Repaired invalid preference value", 0, "PrefsUtility");
			return Boolean.parseBoolean(getUserPrefs().get(ATT_CONSOLIDATEDATABASE,"false"));
		}
	}
	...
	{
		return getRPrefs().get(ATT_RBINARY, "/Library/Frameworks/R.framework/Resources/bin/R");
	}
	
	public static String getRServeBinary()
	{
		return getRPrefs().get(ATT_RSERVEBINARY, "/Library/Frameworks/R.framework/Versions/2.14/Resources/bin/Rserve");
	}
	...
	public static int getRServeStartDelay()
	{
		try
		{
			return (int) (1000*Double.parseDouble(getRPrefs().get(ATT_RSERVESTARTDELAY, "7")));
		}
		catch(Exception e) // Repair the preference
		{
	...
	public static boolean addRepository(Repository rep)
	{
		...
		if (rep.getPath() == null || rep.getPath().equals("")) return false;
	...
	/**
	 * This is called upon opening the prefs window, canceling changes,
	 * or closing the prefs window to ensure that only saved preferences
	 * are used. However, if the user changes the prefs and leaves the window
	 * open without saving, the current changes are used by JEX until canceling or closing
	 */
