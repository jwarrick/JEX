...
public class JEXAvailableDatabases extends JPanel implements ActionListener, MouseListener{
	...
	public JEXAvailableDatabases(JEXDatabaseChooser parent){
		...
		JEXStatics.logManager.log("Connected to database change signal", 0, this);
	...
	public void availableDatabaseChange(){
		JEXStatics.logManager.log("Database list change signal caught", 1, this);
	...
	private void rebuild(){
		JEXStatics.logManager.log("Rebuilding database panel", 1, this);
	...
	public void openNewRepositoryPanel(){
		JEXStatics.logManager.log("Repository creation selected", 1, this);
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) 
        {
			...
			JEXStatics.logManager.log("Repository creation returned "+done, 1, this);
        }
	}
	...
	public void openNewDatabasePanel(Repository rep){
		JEXStatics.logManager.log("Creating new database", 0, this);
	...
	public void openEditDatabasePanel(JEXDBInfo db, Repository rep){
		JEXStatics.logManager.log("Editing database "+db.getDirectory(), 0, this);
...
//		JEXStatics.logManager.log("Selecting database "+dbItem.getDirectory(), 0, this);
...
//		JEXStatics.logManager.log("Opening database "+dbItem.getDirectory(), 0, this);
//		
...
//			if (!isUpToDate)
//			{
//				JEXStatics.logManager.log("Database needs updating", 0, this);
//				int result = JEXReader.update(dbItem);
//				JEXStatics.logManager.log("Database updating returned "+result, 0, this);
//			}
//			else
//			{
//				JEXStatics.logManager.log("Database checked and is ready to open", 0, this);
...
//		}
	...
	class RepositoryPanel extends JPanel implements ActionListener, MouseListener {
		...
		public void reduce()
		{
			JEXStatics.logManager.log("Reducing repository panel", 1, this);
			isExpanded = false;
			rebuild();
		}
		...
		public void expand()
		{
			JEXStatics.logManager.log("Expanding repository panel", 1, this);
			isExpanded = true;
			rebuild();
		}
		...
		private void rebuild()
		{
			JEXStatics.logManager.log("Rebuilding repository panel", 1, this);
			...
			headerpane.setLayout(new BoxLayout(headerpane, BoxLayout.LINE_AXIS));
			headerpane.setBackground(DisplayStatics.lightBackground);
			if (isExpanded) headerpane.add(reduce);
			else headerpane.add(expand);
			headerpane.add(Box.createHorizontalStrut(10));
			headerpane.add(repName);
			headerpane.add(Box.createHorizontalGlue());
			headerpane.add(newDB);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == removeRep){
				JEXStatics.logManager.log("Remove this repertory?", 1, this);
	...
	class DatabasePanel extends JPanel implements ActionListener, MouseListener {
		...
		*/
		private void initialize()
		{
			JEXStatics.logManager.log("Initializing database "+name+" with info "+info, 1, this);
...
//				JEXStatics.logManager.log("Double click on database", 0, this);
...
//			}
		...
		public void actionPerformed(ActionEvent e) 
		{
			if (e.getSource() == editDB){
				JEXStatics.logManager.log("Edit this database", 1, this);
		...
		protected void paintComponent(Graphics g) 
		{
		    ...
		    if (!upToDate) centerColor = new Color(216,177,195);
