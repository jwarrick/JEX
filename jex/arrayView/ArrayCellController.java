...
public class ArrayCellController 
{
	...
	public void rebuild()
	{
		...
		JEXEntry      entry   = null;
		if (cell.getEntries() != null && cell.getEntries().size() > 0) entry = cell.getEntries().iterator().next();
		
		// Is the cell viewed?
		viewed = false;
		if (entry != null){
			JEXEntry viewedEntry = JEXStatics.jexManager.getViewedEntry();
			if (entry.equals(viewedEntry)) viewed = true;
			else viewed = false;
		}
		
		// Is the cell valid?
		TreeMap<String,TreeMap<String,JEXData>> entrymap = entry.getDataList();
		...
		if (labelmap != null){
			...
			if (validityLabelData == null || validityLabelData.getDictionaryValue().equals("true")) valid = true;
			else valid = false;
		}
		...
		if (label != null && label.getName() != null && labelmap!=null){
			...
			// If there is a label of the same name as the one that is selected then change the background
			if (labelValueData!=null) labelColor = JEXStatics.labelColorCode.getColorForLabel(label.getName(),labelValueData.getDictionaryValue());
		}
		
		// Get the selection status
		selected = JEXStatics.jexManager.isAllSelected(cell.getEntries());
		if (selected){
			JEXStatics.logManager.log("Setting border color to selected color", 2, this);
			borderColor = DisplayStatics.selectedDividerColor;
			borderColor = DisplayStatics.selectedButtonBorder;
		}
		else {
			JEXStatics.logManager.log("Setting border color to unselected color", 2, this);
	...
	public void labelSelectionChanged()
	{
		...
		if (label != null && label.getName() != null && labelmap!=null){
			...
			// If there is a label of the same name as the one that is selected then change the background
			if (labeldata!=null) labelColor = JEXStatics.labelColorCode.getColorForLabel(label.getName(),labeldata.getDictionaryValue());
			else labelColor = DisplayStatics.lightBackground;
		}
		else labelColor = DisplayStatics.lightBackground;
		
		//JEXStatics.logManager.log("Label Selection changed, new color for cell "+entry.getTrayX()+"."+entry.getTrayY()+" found is "+labelColor, 1, this);
	...
	public ArrayCellPanel panel()
	{
		if (pane == null)
		{
			if (cell.getEntries() == null) pane = new ArrayCellNULL(this);
			else pane = new ArrayCellPanelSINGLE(this);
		}
		else
		{
			pane.rebuild();
