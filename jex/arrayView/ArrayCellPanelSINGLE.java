...
public class ArrayCellPanelSINGLE extends ArrayCellPanel implements MouseListener
{
	...
	public void rebuild()
	{
		...
		// Viewed status
		if (controller.viewed) this.add(eyeLabel,"width 20,height 20");
		else this.add(noeyeLabel,"width 20,height 20");
		
		// The title and the label color
		if (controller.labelColor == null && temporaryGround == null) titlePane.setBackground(background); 
		else if (controller.labelColor != null || temporaryGround == null) titlePane.setBackground(controller.labelColor);
		else titlePane.setBackground(temporaryGround);
		this.add(titlePane,"growx");
		
		// Valid status
		if (controller.valid) this.add(validLabel,"width 20,height 20,wrap");
		else this.add(invalidLabel,"width 20,height 20,wrap");
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (selected){
			JEXStatics.logManager.log("Removing entry from selection", 1, this);
			JEXStatics.jexManager.removeEntriesFromSelection(controller.entries());
		}
		else {
			JEXStatics.logManager.log("Adding entry to selection", 1, this);
