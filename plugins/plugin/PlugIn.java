...
public class PlugIn extends JDialog implements KeyListener{
	...
	public void dispose()
	{
		if(this.controller != null) this.controller.finalizePlugIn();
		if(JEXStatics.plugins.contains(controller)) JEXStatics.plugins.remove(controller);
		controller = null;
		super.dispose();
		if(JEXStatics.plugins.size() > 0) JEXStatics.plugins.get(0).plugIn().setVisible(true);
		JEXStatics.logManager.log("Disposing PlugIn Window", 0, this);
	}
