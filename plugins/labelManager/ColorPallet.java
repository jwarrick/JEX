...
public class ColorPallet {
	...
	public Color getSelectedColor()
	{
		...
		{
			if(c.isSelected()) return c.getColor();
		}
		return null;
	}
	...
	public void setSelectedColor(String csvColor)
	{
		if(csvColor == null || !hasColor(csvColor))
		{
			...
			JEXStatics.logManager.log("Null string or can't find a matching color in the color pallet", 0, this);
		}
		else
		{
			Color c = stringToColor(csvColor); // returns null if csv isn't formed well (uses try loop in conversion)
			if(c == null)
			{
				// Shouldn't occur because first call to hasColor should catch this condition.
				this.setSelectedColor(-1);
				JEXStatics.logManager.log("Can't find a matching color in the color pallet", 0, this);
			}
			else
			{
				JEXStatics.logManager.log("ColorPallet:setSelectedColor - "+ColorPallet.getColorIndex(c), 0, ColorPallet.class);
	...
	public static boolean hasColor(String csvColor)
	{
		...
		{
			if(color.equals(toMatch)) return true;
		}
		return false;
	}
	
	/**
	 * return the color based on the r,g,b value in the csv string. 
	 * Returns null if something goes wrong in parsing
	 * @param csvColor
	 * @return
	 */
	public static Color stringToColor(String csvColor)
	{
		...
		{
			JEXStatics.logManager.log("ColorPallet:stringToColor - Couldn't conver the csv string to a color!", 0, ColorPallet.class);
		}
		return c;
	}
	...
	public static int getColorIndex(Color color)
	{
		...
		for(Color c : colors)
		{
			if(c.equals(color)) return count;
			count = count + 1;
		}
		return -1;
	}
