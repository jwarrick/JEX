...
public class DatabaseLabelManager {
    ...
    public void _labelSelected()
    {
    	...
    	{
    		JEXStatics.logManager.log("Selected label didn't exist in DBInfo!", 0, this);
    		return;
    	}
    ...
    {    	
    	String name = this.labelNames.getCurrentSelection();
    	String value = this.labelValues.getCurrentSelection();
    	if (name == null || value == null) this.currentLabel = null;
    	else this.currentLabel = new JEXLabel(name, value, "");
    	SSCenter.defaultCenter().emit(this, SIG_ContentsChanged_NULL, (Object[])null);
    }
	...
	{
    	if(this.currentLabel == null) return null;
		...
		if(labels.hasChildNode(nameStr)) label = labels.getChildNode(nameStr);
		else return null;
		if(label.hasChildNode(valueStr)) value = label.getChildNode(valueStr);
		else return null;
    ...
    {
    	if(labelNames.getModel().hasChildNode(text)) return -1;
		int index = this.labelNames.size();
		this.labelNames.getModel().getChildNode(text); // Adds node if not there already
    ...
    {
    	if(!this.labelNames.getModel().hasChildNode(oldText) || this.labelNames.getModel().hasChildNode(newText)) return -1;
    ...
    {
    	if(labelValues.getModel().hasChildNode(text)) return -1;
		int index = this.labelValues.size();
		String csvColor = this.getFirstAvailableColor();
		XPreferences value = this.labelValues.getModel().getChildNode(text); // Adds node if not there already
    ...
    {
    	if(!this.labelValues.getModel().hasChildNode(oldText) || this.labelValues.getModel().hasChildNode(newText)) return -1;
