...
public class ImageAlignerInterface implements PaintComponentDelegate, MouseListener, MouseMotionListener {
	...
	public void paintComponent(Graphics2D g2)
	{
		...
		srcRect = this.delegate.getSrcRect(dstRect); // region of original image being drawn
		displaySrcRect = this.delegate.imageToDisplay(this.display, srcRect); // Actual drawn region including padding but not panel background
		//imageRect = this.delegate.getImageRect(); // Size of original image
		//displayImageRect = this.delegate.imageToDisplay(this.display, imageRect); // Actual drawn region without padding but including panel background
		//intersectionRect = ImageDelegate.intersection(displaySrcRect,displayImageRect); // Actual drawn region without padding and without panel background
		...
		if(this.delegate.getImage() == null) return;
		if(alpha < 1)
		{
	...
	{
		if(!draggable || !(e.getButton() == MouseEvent.BUTTON1)) return;
