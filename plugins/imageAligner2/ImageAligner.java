...
public class ImageAligner implements PlugInController {
	...
	public void imageChanged() // Called each time the entry, DimensionMap;
	{
		...
		if(frontds != null)
		{
			...
			JEXStatics.logManager.log("Opening image at path "+this.frontImageFilePath, 1, this);
		...
		if(backds != null)
		{
			...
			JEXStatics.logManager.log("Opening image at path "+this.backImageFilePath, 1, this);
	...
	public void doubleClick(ImageDelegate delegate, Point displayPoint)
	{
		...
		// Grab the value of the default zoom that fits the image in the display for later use;
		this.leftZoom = this.leftFront.delegate.getZoom();
		...
		this.right.repaint();
		this.middle.repaint();
		// Setting the zoom in the image delegates causes a signal of imageUpdated and statusUpdated
	}
