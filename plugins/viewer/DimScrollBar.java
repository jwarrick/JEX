...
public class DimScrollBar {
	...
	{
		this.scroll = new ScrollPainter(15, this.dim.size(), withToggle);
		this.scroll.setNameText(this.dim.name());
		this.scroll.setVerticalOffset(panelOffset);
		this.scroll.setHeight(panelHeight);
		if(toggleHere == null) toggleHere = JEXStatics.iconRepository.getImageWithName(IconRepository.TOGGLE_HERE, this.scroll.toggleWidth(), this.scroll.toggleWidth());
		if(toggleLeft == null) toggleLeft = JEXStatics.iconRepository.getImageWithName(IconRepository.TOGGLE_LEFT, this.scroll.toggleWidth(), this.scroll.toggleWidth());
		if(toggleRight == null) toggleRight = JEXStatics.iconRepository.getImageWithName(IconRepository.TOGGLE_RIGHT, this.scroll.toggleWidth(), this.scroll.toggleWidth());
		if(toggleAll == null) toggleAll = JEXStatics.iconRepository.getImageWithName(IconRepository.TOGGLE_ALL, this.scroll.toggleWidth(), this.scroll.toggleWidth());
	...
	{
		Dim ret = null;
		if(this.state() == ScrollPainter.ALL) ret = new Dim(this.dim().name(), this.dim().values());
		else if(this.state() == ScrollPainter.LEFT) ret = new Dim(this.dim().name(), this.dim().valuesUpThrough(this.indexInt()));
		else if(this.state() == ScrollPainter.RIGHT) ret = new Dim(this.dim().name(), this.dim().valuesStartingAt(this.indexInt()));
		else ret = new Dim(this.dim().name(), new String[]{this.indexString()});
		return ret;
	}
	...
	public void toggled()
	{
		JEXStatics.logManager.log("toggled", 0, this);
