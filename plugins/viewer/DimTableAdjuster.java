...
public class DimTableAdjuster {
	...
	{	
		bars = new Vector<DimScrollBar>(0);
	...
	{
		if(dimNumber < 0 || dimNumber >= this.bars.size()) return;
	...
	{
		if(tempOverallState == null) tempOverallState = new Vector<Object[]>();
		else tempOverallState.clear();
		DimScrollBar bar;
		for(int i = 0; i < this.bars.size(); i++)
		{
			bar = this.bars.get(i);
			tempOverallState.add(new Object[]{bar.dim().name(), bar.indexInt(), bar.state()});
		}
	}
...
//			this.dimensionMapChanged();
	...
	{
		// JEXStatics.logManager.log("Dimension map changed", 0, this);
	...
	{
		// JEXStatics.logManager.log("Smashed DimTable changed", 0, this);
